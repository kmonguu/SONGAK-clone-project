...
ANTISPAM 3.0부터 제외
ob_start(); 
setcookie("777", "antispam"); 
ob_end_clean(); 
$_SESSION["as7"] = "antispam";
*/
...
if ($wr_id)
{
    ...
    if ($group[gr_use_access])
    {
        ...
        else
        {
            ...
            $row = sql_fetch($sql);
            if (!$row[cnt])
                alert("접근 권한이 없으므로 글읽기가 불가합니다.\\n\\n궁금하신 사항은 관리자에게 문의 바랍니다.", $g4[mpath]);
        }
    }
    ...
    $g4[title] = "{$g4['title']}" . strip_tags(conv_subject($write[wr_subject], 255));
}
else
{
    ...
    $g4[title] = "$board[bo_subject] - $group[gr_subject] ";
}
...
if (!($board[bo_use_comment] && $cwin)) {
    ...
    if ($member[mb_level] >= $board[bo_list_level] && $board[bo_use_list_view] || empty($wr_id))
     ...
     include_once($g4[mpath]."/bbs/board_tail.php");
}
else

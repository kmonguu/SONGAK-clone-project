...
@include_once("$board_skin_mpath/view.head.skin.php");
...
$prev_href = "";
if ($prev[wr_id]) {
    $prev_wr_subject = get_text(strcut_utf8($prev[wr_subject], 50));
...
$next_href = "";
if ($next[wr_id]) {
    $next_wr_subject = get_text(strcut_utf8($next[wr_subject], 50));
...
$view = get_view($write, $board, $board_skin_mpath, 255);
...
$view[content] = preg_replace("/(\<img )([^\>]*)(\>)/i", "\\1 name='target_resize_image[]' onclick='image_window(this)' \\2 \\3", $view[content]);
...
include_once("$board_skin_mpath/view.skin.php");

@include_once("$board_skin_mpath/view.tail.skin.php");
?>

...
{
    if ($cwin) // 코멘트 보기
       alert_close("존재하지 않는 게시판입니다.", $g4[mpath]);
    else
       alert("존재하지 않는 게시판입니다.", $g4[mpath]);
}

if (!$bo_table)
    alert("bo_table 값이 넘어오지 않았습니다.\\n\\nwrite.php?bo_table=code 와 같은 방식으로 넘겨 주세요.", $g4[mpath]);

@include_once ("$g4[mpath]/skin/board/write.head.skin.php");
@include_once ("$board_skin_mpath/write.head.skin.php");
...
if ($group[gr_use_access])
{
    ...
    if ($is_admin == 'super' || $group[gr_admin] == $member[mb_id] || $is_admin == "board")
        ; // 통과
    else {
        // 그룹접근
...
$g4[title] = "$board[bo_subject] - " . $title_msg. " - $group[gr_subject]";

if (($w == "u" || $w == "r") && !$write[wr_id])
    alert("글이 존재하지 않습니다.\\n\\n삭제되었거나 이동된 경우입니다.", $g4[mpath]);
...
if ($w == "")
    $password_required = "data-required='required'";
else if ($w == "u") {
    $password_required = "";
...
include_once($g4[mpath]."/bbs/board_head.php");
...
include_once ("$board_skin_mpath/write.skin.php");
...
// author : Ko gi boong
// DESC : /bbs/write.php 하단 include('./board_tail.php') 코드 위
//=====================================================================================================================================
*/
$curl = curl_init();
...
@include_once ("$board_skin_mpath/write.tail.skin.php");
?>

<!-- MobileEdit -->
<link rel="stylesheet" href="/m/skin/board/MobileEdit.css" type="text/css"> 
<script type="text/javascript" src="/m/skin/board/MobileEdit.js"></script>
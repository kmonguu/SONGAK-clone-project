...
function err($str) {
    echo "<div id='ifrErr'>$str</div>";
    exit;
}

if (!trim($mb_id) || !trim($mb_password))
    err("회원아이디나 패스워드가 공백이면 안됩니다.");
...
if ($check_time = get_session("ss_login_check_time")) {
    if ($check_time > $g4['server_time'] - 15) {
        err("로그인 실패시에는 15초 이후에 다시 로그인 하시기 바랍니다.");
    }
}
...
//로그인 기록
sql_query("INSERT INTO g4_login_log SET mb_id='{$mb["mb_id"]}', login_date=now()");

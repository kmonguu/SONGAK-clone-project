...
<tr>
    ...
    <td bgcolor=#FAFAFA class="od_group"  style='padding-left:10px' colspan="2">
        ...
        <div class="od_group_title">
            ...
            <label>
				<input type=checkbox onclick="javascript:gumae2baesong2(this, document.forderform);">
				<b style='font-size:16px;'>주문하시는 분과 정보가 동일한 경우 체크</b>
			</label>
        </div>
        ...
        <tr>
            <td>이름</td>
            <td><input type=text class="md_b_name ed" name="md_b_name[<?=$cnt?>]" maxlength=20 maxlength=20 style="width:200px;"></td>
        </tr>
        <tr style="display:none;">
            <td >전화번호</td>
            <td><input type=text class="md_b_tel ed" name="md_b_tel[<?=$cnt?>]" maxlength=20 maxlength=20 style="width:200px;" ></td>
        </tr>
        <tr>
            <td>핸드폰</td>
            <td><input type=text class="phonenum md_b_hp" name="md_b_hp[<?=$cnt?>]" class=ed maxlength=20 maxlength=20 style="width:200px;"></td>
        </tr>
        <tr>
            <td rowspan=2>주 소</td>
            <td>
            	<div style="float:left;">
	                <input type=text class="md_b_zip1 ed" name="md_b_zip1[<?=$cnt?>]" style="width:80px;"  maxlength=5 readonly>
            	</div>
        ...
        <tr>
            <td>
                <input type=text class="md_b_addr1 ed" name="md_b_addr1[<?=$cnt?>]" style="width:90%;" maxlength=50 readonly>
                <input type=text class="md_b_addr2 ed" name="md_b_addr2[<?=$cnt?>]" style="width:90%;margin-top:5px;" maxlength=50 > (상세주소)
            </td>
        </tr>
        <tr>
            <td>전하실말씀</td>
            <td><input type="text" name="md_memo[<?=$cnt?>]" rows=1 style="width:90%; " class=ed/></td>
        </tr>
        ...
        <tr>
            <td style='padding:8px 2px;'> 
				...
				<?if($cnt == 1) {?>
					<input type="hidden" class="tot_qty" data-key="<?=$ct["it_id"]?>" value="<?=$ct["ct_qty"]?>"/>
				<?}?>
				...
				<span style="display:inline-block; min-width:200px;">
                    <span style="font-weight:bold;"><?=$ct["it_name"]?></span><br/><span style='font-color:#9d9d9d'><?=$it_option_str?></span> <?=$it_add_options?>
				</span>

			
			</td>
            <td>
                 <select name="md_qty[<?=$cnt?>][]" class="qty_<?=$ct["it_id"]?>">

...
.loginbtn2 span.loginbtn1 {float:left;width:104px;height:30px;background:#efefef;color:#2d2d2d;text-align:center;font-size:14px;line-height:29px;text-decoration:none;cursor:pointer;}
.loginbtn2 a:active span {color:#2d2d2d;background:#000000;}
.loginbtn2 {float:right;margin:15px 0 15px 0;padding:0 0 0 0;}


.Surveytit03 {float:left;color:#1f1f1f;font-size:19px;font-weight:bold;padding:0 0 0 32px;background:url("/res/images/total_ico04.jpg") no-repeat 6px 4px;line-height:21px;margin:10px 0 0 0;width:100%;}
table.b7 {width:100%;border:0;background:#fff;padding:0 0 0 0;}
table.b7 th {padding: 0;font-size:15px;background:#f4f4f4;font-weight:bold;color:#1f1f1f;height:38px;border-top:1px solid #455560;border-bottom:1px solid #455560;}
table.b7 td {border-bottom:1px solid #c7cccf;padding:5px 0px; text-align:center;vertical-align:middle;color:#5e5e5e;background:#fff; font-size:15px;}
...
.coupons {font-size:16px; padding:10px 5px;background-color:white; border:1px solid #afafaf; box-shadow:#efefef 1px 1px 1px;}
</style>


<p style="height:62px;line-height:60px;font-size:24px;font-weight:bold;color:#0c0c0c; width:100%; box-sizing:border-box; padding:0 20px; border-top:3px solid #222; border-bottom:2px dashed #eee; ">쿠폰 사용하기</p>


<div class="Surveytit03" style='width:90%;'>장바구니 목록</div>

<div class="Boradnlist Listlink" style="float:left;margin-top:10px; padding:10px 10px 10px 10px; width:620px;">
	<?
		$cart_list = $ycart->list_cart($on_uid);
	?>
	
	<table cellspacing="0" cellpadding="0" class="b7" summary="주문내역 입니다." style="table-layout:fixed;">
		<colgroup>
			<col />
			<col width="120px"/>
			<col width="160px"/>
		...
		<tbody>
			...
			<tr>
				...
				<td class="<?=$cls?>">
						<select name='coupon' class="coupons" style='width:150px;' onchange="change_coupon(this.value, '<?=$it[it_id]?>', '<?=$row[ct_id]?>')">
				...
				<td class="<?=$cls?>" style='font-size:20px; text-align:right; padding-right:5px;'>
                    ...
                    <span style="color:#0000CC;" id="disp_discount_<?=$row[ct_id]?>"></span><br/><span style="color:blue;" id="disp_amount_<?=$row[ct_id]?>"></span>
				</td>
			...
			<tr >
				<td colspan="4" style='text-align:right;padding-right:15px;'>
						...
						<div class="loginbtn2" style="margin-right:10px;">
							<a href="javascript:void(0)" onclick='top.$.fancybox.close();'><span class="loginbtn1">닫기</span></a>
						</div>
...
<script type="text/javascript">
	...
	function change_coupon(cpn_id, it_id, ct_id){
		...
		if(cpn_id == ""){
			...
			$.post("/shop/_ajax_cpn_amount.php", "it_id="+it_id + "&cpn_id="+cpn_id + "&ct_id="+ct_id, function(data){

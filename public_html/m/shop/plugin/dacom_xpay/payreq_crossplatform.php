<?php
     ...
     */
    // $CST_PLATFORM               = "service";//$HTTP_POST_VARS["CST_PLATFORM"];      //LG유플러스 결제 서비스 선택(test:테스트, service:서비스)
    ...
    $LGD_CUSTOM_SKIN            = "SMART_XPAY2";                        //상점정의 결제창 
    $LGD_ENCODING            = "UTF-8";                        //상점정의 결제창 스킨
    

	// 추가 파라미터
	$LGD_BUYERPHONE			= $HTTP_POST_VARS["LGD_BUYERPHONE"];    //구매자 핸드폰번호
	$on_uid						= $_SESSION['ss_temp_on_uid'];    //uid
     ...
     */    
    $LGD_CASNOTEURL				=  $http.$_SERVER[HTTP_HOST]."/m/shop/plugin/dacom_xpay/cas_noteurl.php";    

    /*
     * LGD_RETURNURL 을 설정하여 주시기 바랍니다. 반드시 현재 페이지와 동일한 프로트콜 및  호스트이어야 합니다. 아래 부분을 반드시 수정하십시요.
     */    
    $LGD_RETURNURL				=  $http.$_SERVER[HTTP_HOST]."/m/shop/plugin/dacom_xpay/returnurl.php";  
	
    /*
     * ISP 카드결제 연동중 모바일ISP방식(고객세션을 유지하지않는 비동기방식)의 경우, LGD_KVPMISPNOTEURL/LGD_KVPMISPWAPURL/LGD_KVPMISPCANCELURL를 설정하여 주시기 바랍니다. 
     */    
    $LGD_KVPMISPNOTEURL       	= $http.$_SERVER[HTTP_HOST]."/m/shop/plugin/dacom_xpay/note_url.php";
    //$LGD_KVPMISPWAPURL			= "http://".$_SERVER[HTTP_HOST]."/m/shop/plugin/dacom_xpay/lgumobile/mispwapurl.php?LGD_OID=".$LGD_OID;   //ISP 카드 결제시, URL 대신 앱명 입력시, 앱호출함 
    $LGD_KVPMISPCANCELURL     	= $http.$_SERVER[HTTP_HOST]."/m/shop/plugin/dacom_xpay/cancel_url.php";
    ...
    $xpay = &new XPayClient($configPath, $LGD_PLATFORM, $LGD_MERTKEY);
	...
	$payReqMap['LGD_ENCODING']  = $LGD_ENCODING;	       // 신용카드 카드사 인증 페이지 연동 방식
	
	$payReqMap['LGD_BUYERPHONE']		= $HTTP_POST_VARS["LGD_BUYERPHONE"];    //구매자 핸드폰번호
	$payReqMap['on_uid']						= $_SESSION['ss_temp_on_uid'];    //uid
	$payReqMap['mb_id']						= $member[mb_id];    //회원아이디
    ...
    */
    ...
    $payReqMap['LGD_KVPMISPAUTOAPPYN'] = $LGD_KVPMISPAUTOAPPYN;               // 가상계좌 NOTEURL

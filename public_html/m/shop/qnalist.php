...
<!--
<div>
<a href="/bbs/write.php?bo_table=5_2_1_1"><img src="/res/images/review_img.jpg" /></a>
</div>
-->
	...
	<div class="RoomBox02In" style="border:1px solid #fff;">
		<div style="float:left;width:100%;padding:0 0 10px 0;">
			...
			<div style="float:right;">
				<a href="/m/bbs/write.php?bo_table=<?=$qna_board?>&item_id=<?=$it_id?>" class="afterlist_btn" ><span>질문하기</span></a>&nbsp;&nbsp;
				<a href="/m/bbs/board.php?bo_table=<?=$qna_board?>&cate_id=<?=$it[ca_id]?>&sop=and&sca=" class="afterlist_btn" ><span>+ 더보기</span></a>
			</div>
		</div>
		<?
		...
		<div style="float:left; width:100%; height:24px; line-height:24px; border-top:1px solid #d6d6d6; padding:10px 0;background:#f6f6f6; vertical-align:top;">
			<div style="float:left;width:30%;;color:#000; height:24px; line-height:24px; vertical-align:top;">&nbsp;&nbsp;<?=$bbs_list[$b][name]?>&nbsp;&nbsp;<span style="font-size:15px;;color:#000; vertical-align:top;"><?=$bbs_list[$b][datetime]?></span></div>
			<div style="float:left;width:70%;color:#4a4a4a; text-overflow:ellipsis; overflow:hidden; white-space:nowrap; height:24px; line-height:24px; vertical-align:top;">
				<a href='javascript:void(0)' onclick="<?=$goScript?>" style="<?if(!$is_secret) {?>color:#000;<?}else{?>color:#aaa;<?}?>">
			...
			<?} else {?>
				...
				<div style="width:100%;text-align:right;">
					<?if($is_admin && !$bbs_list[$b][wr_reply]){?>
						<a href="/m/bbs/write.php?w=r&bo_table=<?=$qna_board?>&wr_id=<?=$bbs_list[$b][wr_id]?>" style="color:#000"><span class='btn1-o'>답변하기</span></a>
					<?}?>
					<a href="/m/bbs/board.php?bo_table=<?=$qna_board?>&wr_id=<?=$bbs_list[$b][wr_id]?>" style="color:#000"><span class='btn1'>상세/수정/삭제</span></a>
				</div>
...
<script>
	...
	function go_qna_view(wr_id){
		location.href = "<?=$g4["mbbs_path"]?>/board.php?bo_table=<?=$qna_board?>&wr_id="+wr_id;
	}
</script>

...
<tr class="option_item option_item_<?=$it_option["no"]?>"  data-qty="<?=$qty?>" >
	...
	<td colspan="2" >
		...
		?>
			...
			<div class="opt_name" style="position:relative; width:100%;" data-name="<?=$opt_name?>" >
				<i class="fas fa-gift"></i>
				<?=$opt_name?>

				<!-- 옵션 삭제 버튼-->
				<div style="position:absolute; top:9px; right:0px; font-size:18px; color:#CC0000; font-weight:bold; margin-right:10px; cursor:pointer;" onclick="delete_option_item(this);">	
		...
		<?if($qty > 0) {
			...
			<div class="opt_name vright_con_title" data-name="<?=$it["it_name"]?>" style="width:auto;" >
				<!--
			...
			<div style="float:right;" >
				...
				<input type=number name='ct_qty[]' value='<?=$ct["ct_qty"]?>' maxlength=4 class="ed ct_qty" autocomplete='off' style='text-align:center; width:50px; height:36px; border:1px solid #cecece; float:left; padding:0;' onkeyup='qty_keyup(this)'>
				<img src="/shop/img/plus_btn.jpg" onclick="qty_add(this, +1)" style="cursor:pointer; float:left;" /> -->

				<input type=number name='ct_qty[]' value='<?=$ct["ct_qty"]?>' maxlength=4 class="ed ct_qty" autocomplete='off' style='text-align:center; width:60px; height:36px; border:1px solid #cecece; float:left; padding:0; margin-right:5px; ' onkeyup='qty_keyup(this)'>
				...
				<?if($is_opt) {?>
					...
					<div class="opt_amount" style="min-width:auto;" >

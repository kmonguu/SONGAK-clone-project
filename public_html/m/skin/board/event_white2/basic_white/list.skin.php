...
<div id="m_list">
	...
	?>
		...
		<li <?=$list_selected?> onclick="location.href='<?=$g4[mpath]?>/bbs/board.php?bo_table=<?=$bo_table?>&wr_id=<?=$list[$i][wr_id]?>&page=<?=$page?>'"  style="height:38px;">
			<div style='height:30px;width:70%;overflow:hidden;float:left;'>
				<a class="list_left" style="font-size:24px;"><?=$list[$i][subject]?></a>
				<?=$icon_secret?><?=$icon_new?> <?=$wr_comment?>
			</div>
			<nobr><a class="list_right" style="font-size:24px; width:27%;"><?=$list[$i][wr_name]?></a></nobr>
		</li>
		<?}?>
		<? if($i==0){ 	?>

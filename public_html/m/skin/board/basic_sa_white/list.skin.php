...
<table width="<?=$width?>" align="center" cellpadding="0" cellspacing="0"><tr><td>
       ...
       // 분류사용, 상품사용하는 상품의 정보를 얻음
		$sql = " select a.*,
				  ...
				  where a.it_id = '".$list[$i][wr_6]."'
					and a.ca_id = b.ca_id ";
		$it = sql_fetch($sql);
		$href = "$g4[shop_mpath]/item.php?it_id=$it[it_id]";
    ...
    <tr class="bg<?=$bg?>">
		...
		<td class="img" style="padding:5px;" >
			<?if($it[it_name]){?>
				<a href="<?=$href?>"><?=get_it_image2($it[it_id], '_s',70,70);?></a>
			<?}else{?>
				&nbsp;
			<?}?>
		</td>
        <td class="subject">
			...
			$lname = strcut_utf8(strip_tags($listname), 23);

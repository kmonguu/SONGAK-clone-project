...
<tr>
    ...
    <td style="padding:10px;">
		<input type="checkbox" name=wr_10 id="wr_10" itemname="메인노출" value="1" <?=$write[wr_10]?"checked":""?> onclick="change_wr_10()" ><label for="wr_10">메인노출</label>
	</td>
</tr>
<tr><td colspan=2 height=1 bgcolor=#e7e7e7></td></tr>

<tr>
    <td class=write_head>유튜브<br/>동영상ID</td>
    <td style="padding:10px;"><input class='ed' style="width:99%;"  name=wr_1 id="wr_1" itemname="url" required value="<?=$write[wr_1]?>"></td></tr>
<tr><td colspan=2 height=1 bgcolor=#e7e7e7></td></tr>
...
<div style="color:#ffffff; width:60px; text-align:center; padding:5px; background:#CC0000; font:11px dotum; font-weight:bold; margin-top:15px; position: relative;" id="Howtouse">사용방법
<div style="color:#000000; background:#ffffff; border:2px solid #CC0000; padding:10px;  width:200px; font:11px dotum; position:absolute; top:-140px;left:73px; display:none; text-align:left;line-height:1.3;" id="information">
	1. 뷰화면에서 나올 동영상의 ID를 입력하세요. <br />
	( <span style="color:#7b7b7b">https://youtu.be/</span><span style="color:#ff5f5f;">m8fxevYRkMY</span> )<br />
	2. 제목을 입력하세요.<br />
	3. 내용을 입력하세요.<br />
	4. 파일첨부를 통해 목록에 나올 이미지를 넣어주세요. 첨부하지 않을 경우 해당 동영상의 썸네일이 자동 등록됩니다.<br />
	5. 썸네일 이미지는 가로220px 세로156px로 맞춰주세요</div>
</div>
...
function change_wr_10()
{
	var wr_10_val = $("#wr_10").is(":checked");

	if (wr_10_val) {
		$.post("<?=$board_skin_path?>/_ajax_check_wr_10.php", {bo_table:"<?=$bo_table?>"}, function(data){
			if(data >= 1){
				alert("메인노출은 최대 1개까지 가능합니다. 다른 게시글의 메인노출 해제 후 이용해주세요.");
				document.fwrite.wr_10.checked = "";
			}
		});
	}
}

	...
	function statusChangeCallback(response) {
		...
		if (response.status === 'connected') {
			...
			FB.api('/me', 'get', { access_token: response.authResponse.accessToken, fields: 'id,name,email' }, function(res) {
				...
				document.fregisterform.mb_facebook_id.value = response.authResponse.userID;
				document.fregisterform.mb_id.value = response.authResponse.userID + "@facebook";
				document.fregisterform.mb_name.value = res.name;
				document.fregisterform.email1.value = res.email;
				$("#email1").blur();

				$(".fb_loading").fadeOut("slow");
			});

		} else {
			alert("Facebook 아이디로 회원 가입에 실패했습니다.");
			location.href = "/";
...
<div class="fb_loading" style="position:fixed; top:0px; left:0px; width:100%; height:100%; z-index:999; background-color:black; opacity:0.5;">
	<div style="width:100%; position:fixed; top:30%; left:0px; font-size:40px; color:white; text-align:center; ">
		잠시만 기다려주세요...
	</div>
</div>
...
<input type="hidden" name="mb_is_facebook" value="1" />
<input type="hidden" name="access_token" value="<?=$access_token?>" />
<input type="hidden" name="mb_facebook_id" value="" />
...
<table width="600" cellspacing="0" cellpadding="0" class="regiter_table" style="margin-left:20px;">
    ...
    <tr bgcolor="#FFFFFF" style="display:none;">
		...
		<td class=m_padding>
			<input type=text class=ed maxlength=20 type=text size=20 required id='reg_mb_id' name="mb_id" readonly value="" />
	...
	<tr bgcolor="#FFFFFF"  style="display:none;">
		...
		<td class=m_padding>
			<INPUT class=ed type=password name="mb_password" size=20 maxlength=20 <?=($w=="")?"required":"";?> itemname="패스워드" value="<?=md5($access_token)?>">
	...
	<?
	if($w == "") $member["mb_email"] = $user->facebook_account->email;
	?>
	...
	<TR bgcolor="#FFFFFF">
		...
		jQuery(window).ready(function(){
			...
			jQuery("#email2, #email1").bind("blur",function(){
				...
				jQuery("#reg_mb_email").val( jQuery("#email1").val() + "@" + jQuery("#email2").val() ).trigger('blur');
			});

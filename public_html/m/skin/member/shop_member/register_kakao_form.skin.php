...
.regiter_table td { padding:5px 0 5px 20px; border-bottom:1px solid #c2c2c2; font-size:16px; color:#8e8e8e; font-weight:300; height:42px; }
.regiter_table td.m_title { background:#f9fafa; color:#504f4f; font-weight:400; font-size:20px; }
.regiter_table td input.ed { border:1px solid #e0e0e0; outline:none; padding:10px; width:234px; height:46px; }
.regiter_table td select { border:1px solid #e0e0e0; outline:none; padding:10px; width:234px; }
...
<input type="hidden" name="mb_is_kakao" value="1" />
<input type="hidden" name="access_token" value="<?=$access_token?>" />
<input type="hidden" name="mb_kakao_id" value="<?=$w=="" ? $user->id : $member["mb_kakao_id"]?>" />

<!-- ANTISPAM -->
<input type=hidden name=bo_table value="register_form"/>

<p style="color:#000000; font-size:28px; padding:15px 25px 10px;">정보입력</p>
<p style="color:#848484; font-size:22px; font-weigth:300; padding:0px 25px;">회원님의 개인정보를 안전하게 보호하고 있으며, 회원님의 명백한 동의 없이는 공개 또는 제3자에게 제공되지 않습니다.</p>

<p style="color:#000000; font-size:28px; padding:35px 25px 10px;">*필수 정보입력</p>
<table width="600" cellspacing="0" cellpadding="0" class="regiter_table" style="margin-left:20px;">
	<colgroup>
		<col width="150px" />
		<col width="" />
	</colgroup>
	...
	<tr bgcolor="#FFFFFF" style="display:none;">
		...
		<td class=m_padding>
			<input type=text class=ed maxlength=20 type=text size=20 required id='reg_mb_id' name="mb_id" readonly value="<?=$user->id?>@kakao" />
	...
	<tr bgcolor="#FFFFFF">
		...
		<td class=m_padding>
			<input type=text class=ed name=mb_name required itemname="이름" value="" <?=$member[mb_name]?"readonly":""?>>
    ...
    <? if ($w=="") { 
        if($user->kakao_account->has_birthday){
            $member["mb_birth"] = date("Y-m-d", strtotime(date("Y").$user->kakao_account->birthday));
        }
        ?>
        <tr bgcolor="#FFFFFF">
	...
	<TR bgcolor="#FFFFFF">
		...
		<TD class=m_padding>
			<!--휴대폰 입력부 -->
			<select id="hp1" class="hp_input" <?=$config[cf_req_hp]?'required':'';?> style="width:78px;" >
			...
			-
			<input type="number" class="ed hp_input" maxlength=4 id="hp2" style="width:61px;" <?=$config[cf_req_hp]?'required':'';?> value="<?=$mb_hp2?>"> -
			<input type="number" class="ed hp_input" maxlength=4 id="hp3" style="width:61px;" <?=$config[cf_req_hp]?'required':'';?> value="<?=$mb_hp3?>">
	...
	<?
    if($w == "") 
        if($user->kakao_account->has_email){
            $member["mb_email"] = $user->kakao_account->email;
        }
	?>
	<input type=hidden name='old_email' value='<?=$member[mb_email]?>'>
	<TR bgcolor="#FFFFFF">
		<TD class=m_title><span class="orange_star">* </span>이메일</TD>
		<TD class='m_padding lh' style="line-height:3.5;">
		...
		<select id="email_selector" style="width:120px;">
	...
	<TR bgcolor="#FFFFFF">
		...
		<TD class='m_padding lh'>
			<input class=ed type=text required id='reg_mb_nick' name='mb_nick' maxlength=20 value='<?=$user->properties->nickname?>'
	...
	<?if($g4["use_mb_sex"]){?>	
		...
		<? } else { ?>
			...
			<?
				$mbsex = "";
				if($user->kakao_account->has_gender){
					$mbsex = $user->kakao_account->gender;
				}
			?>
			<TR bgcolor="#FFFFFF">
				<TD class=m_title><span class="orange_star">* </span>성별</TD>
				<TD class=m_padding>
					<label><input type="radio" id=mb_sex_m name=mb_sex required value='M' itemname='성별' <?=$mbsex=="" || $mbsex=="male" ? "checked" : ""?> > 남자</label>
					<label><input type="radio" id=mb_sex_f name=mb_sex required value='F' itemname='성별' <?=$mbsex=="female" ? "checked" : ""?> > 여자</label>
...
<p style="color:#000000; font-size:28px; padding:35px 25px 10px;">선택 정보입력(알림설정)</p>
<table width="600" cellspacing="0" cellpadding="0" class="regiter_table" style="margin-left:20px;">
	<colgroup>
		<col width="150px" />
		<col width="" />

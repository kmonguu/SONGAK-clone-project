...
<p style="color:#000000; font-size:20px; padding:0px 0 10px;">정보입력</p>
...
<table width="100%" cellspacing="0" cellpadding="0" class="regiter_table" >
	...
	<tr bgcolor="#FFFFFF">
		...
		<td class=m_padding>
			<input type=text class=ed name=mb_name required itemname="이름" value="<?=$member[mb_name]?>" <?=$member[mb_name]?"readonly":""?>>
	...
	<tr bgcolor="#FFFFFF">
		...
		<td class=m_padding>
			<input type=text class=ed maxlength=20 type=text size=20 required id='reg_mb_id' name="mb_id" value="<?=$member[mb_id]?>" <? if ($w=='u') { echo "readonly style='background-color:#dddddd;'"; } ?>
				<? if ($w=='') { echo "onblur='reg_mb_id_check();'"; } ?> >
			<span style="margin-left:15px;" ><span id='msg_mb_id'> * 영문자, 숫자, _ 만 입력 가능. 최소 3자이상 입력하세요.</span></span>
		</td>
	</tr>
	<tr bgcolor="#FFFFFF" style="<?=($member["mb_is_naver"] || $member["mb_is_kakao"] || $member["mb_is_facebook"])? "display:none" : "";?>" >
		<td class=m_title><span class="orange_star">* </span>패스워드</td>
		<td class=m_padding>
			<INPUT class=ed type=password name="mb_password" size=20 maxlength=20 <?=($w=="")?"required":"";?> itemname="패스워드">
			<span style="margin-left:15px;" >* 영문+숫자가 조합된 문자를 입력해 주세요. 최소 8자이상</span>
		</td>
	</tr>
	<tr bgcolor="#FFFFFF" style="<?=($member["mb_is_naver"] || $member["mb_is_kakao"] || $member["mb_is_facebook"])? "display:none" : "";?>">
		<td class=m_title><span class="orange_star">* </span>패스워드 확인</td>
		<td class=m_padding>
			<INPUT class=ed type=password name="mb_password_re" size=20 maxlength=20 <?=($w=="")?"required":"";?> itemname="패스워드 확인">
			<span style="margin-left:15px;" >* 비밀번호를 한번 더 입력해 주세요.</span>
		</td>
	</tr>
	...
	<TR bgcolor="#FFFFFF">
		...
		<TD class=m_padding>
			...
			-
			<input type="number" class="ed hp_input"  maxlength=4 id="hp2" oninput="maxLengthCheck(this)" style="width:78px; text-align:center;"  <?=$config[cf_req_hp]?'required':'';?> value="<?=$mb_hp2?>"> -
			<input type="number" class="ed hp_input"  maxlength=4 id="hp3" oninput="maxLengthCheck(this)" style="width:78px; text-align:Center"  <?=$config[cf_req_hp]?'required':'';?> value="<?=$mb_hp3?>">
	...
	<TR bgcolor="#FFFFFF">
		...
		<TD class='m_padding lh'>
			<input class=ed type=text required id='reg_mb_nick' name='mb_nick' maxlength=20 value='<?=$member[mb_nick]?>'
	...
	<? 
	//주소 사용 하려면 관리자 환경설정에서 "주소 입력" 체크 하세요
	...
	<tr bgcolor="#FFFFFF">
		<td class=m_title><span class="orange_star">* </span>보안코드</td>
		<td class=m_padding>
			<img id='kcaptcha_image' style="vertical-align:middle;" />
			<input type=input class=ed name=wr_key itemname="자동등록방지" required style="margin-left:15px;" >
			<span style="margin-left:15px;" >* 왼쪽의 보안코드 문자를 입력해 주세요.</span>
		</td>
	</tr>
...
<div style="padding-top:30px; text-align:center;">
	<?if($w=="u"){?>
		<span class="btn1-o" style="cursor:pointer;float:left;" onclick="member_leave()" >회원탈퇴</span>
...
function fregisterform_submit(f)
{
	...
	if (f.w.value == '' || pw.length > 0) {
	 ...
	 if(num < 0 || eng < 0 ){
		  ...
		  alert("패스워드를 영문,숫자를 혼합하여 입력해주세요.");

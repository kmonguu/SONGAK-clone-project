...
<table width="100%" cellspacing="0" cellpadding="0" class="regiter_table" >
    ...
    <? if ($w=="") { 
        if($user->kakao_account->has_birthday){
			...
			$birth_year = date("Y", strtotime(date("Y").$user->kakao_account->birthday));
			$birth_month = date("m", strtotime(date("Y").$user->kakao_account->birthday));
			$birth_day = date("d", strtotime(date("Y").$user->kakao_account->birthday));
        }
        ?>
        <tr bgcolor="#FFFFFF">

...
<tr>
    ...
    <td style="padding:10px;"><span id='item_list'><?=$item_list?></span><input type='hidden' id='wr_7' name='wr_7' value='<?=$write[wr_7]?>'/></td></tr>
<tr><td colspan=2 height=1 bgcolor=#e7e7e7></td></tr>
<!-- 상품 선택 끝-->
<? if ($is_name || ($is_admin && $w == "")) { ?>
<tr>
...
<tr>
    ...
    <td style="padding:10px;"><input class='ed' maxlength=20 size=25 name=wr_datetime itemname="이름" required value="<?=date("Y-m-d H:i:s")?>"></td></tr>
<tr><td colspan=2 height=1 bgcolor=#e7e7e7></td></tr>
<? } ?>
...
<tr>
    <td width="100%" align="center" valign="top" style="padding-top:25px;">
        ...
        <?if($item_id || $write["wr_6"]){?>
            <a href="/shop/item.php?it_id=<?=$item_id ?  $item_id : $write["wr_6"]?>#qnalist" class="nBtn2">상품으로</a>
        <?} else {?>

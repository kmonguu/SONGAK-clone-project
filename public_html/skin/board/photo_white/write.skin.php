...
.WriteBoardTable > tbody > tr > th { height:45px; font-size:15px; text-align:center; color:#6e6e6e; border-right:1px solid #e6e6e6; border-bottom:1px solid #e6e6e6; background:#fafafa; font-weight:400; }
.WriteBoardTable > tbody > tr > td { font-size:14px; text-align:left; color:#6e6e6e; border-bottom:1px solid #e6e6e6; padding:10px; }
.field { border:1px solid #ccc; }

#variableFiles td { padding:2px 0; }
.file_btn { background:none; border:0; font-size:16px; margin:0 5px; outline:none; color:#777; }
.file_btn:hover { color:#222; }
.file_input { width:250px; height:23px; border:1px solid #ddd; background:#fff; box-shadow:none; }
.file_input_content { vertical-align:top; width:400px; margin-left:5px; padding:0 5px; }

a.nBtn1,
.nBtn1 { background:#484848; color:#ffffff; padding:7px 15px; line-height:23px; font-size:14px; font-weight:400; display:inline-block; text-decoration:none; border:0px; }
a.nBtn2,
.nBtn2 { background:#727272; color:#ffffff; padding:7px 15px; line-height:23px; font-size:14px; font-weight:400; display:inline-block; text-decoration:none; border:0px; }
...
<div style="height:50px; line-height:50px; background:#f8f8f8; padding:0 10px; width:100%; box-sizing:border-box; font-size:17px; border-top:1px solid #dddddd; border-bottom:1px solid #dddddd; color:#6e6e6e;">
<?=$title_msg?>
</div>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="WriteBoardTable" >
	...
	<tbody>
		...
		<tr>
			...
			<td>
				<input class='ed' maxlength=20 size=15 name=wr_name id="wr_name" itemname="이름" data-required="required" value="<?=$name?>" >
			</td>
		</tr>
		<? } ?>
		...
		<? if ($is_password) { ?>
			...
			<? if($board["bo_use_secret"] > 0 || (isset($bo_use_password_input) && in_array($bo_table, $bo_use_password_input))){ ?>
				<tr class="tr_password" style="display:none;" >
					...
					<td>
						<input type="password" class='ed' maxlength=20 size=15 name=wr_password id="wr_password" itemname="패스워드" <?=$password_required?> value="<?=md5(date("YmdHis"))?>" >                
					</td>
				</tr>
				<script>
		...
		<tr>
			<th scope="row" ><label for="wr_content">본 문</label></th>
			<td>
				<? if ($is_dhtml_editor) { ?>
		...
		<tr>
			<th scope="row" ><label for="wr_content">썸네일</label></th>
			<td>
...
<? if ($is_file && $board[bo_upload_count] > 0) { ?>
	<p style="color:#CC0000; font:11px dotum; margin-top:5px;">첨부파일 이미지 사이즈는 가로 최대 1200px로 맞춰주세요.</p>
<?}?>

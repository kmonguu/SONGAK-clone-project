...
//장바구니 상품을 일괄적으로 회원 가격으로 변경
function set_cart_member_amount($on_uid){

	global $g4;
	$sql = " SELECT * FROM $g4[yc4_cart_table] WHERE on_uid = '{$on_uid}' ";
	$result = sql_query($sql);
	for($idx = 0 ; $row = sql_fetch_array($result); $idx++){
		$it = sql_fetch("SELECT * FROM $g4[yc4_item_table] WHERE it_id='{$row["it_id"]}' ");		
		$newAmt = get_amount($it)."<br/>";
		$newPoint = intval($row["ct_point"]);
		//추가옵션가격
		for($oi = 1;  $oi <=6 ; $oi++){
			$opt = $row["it_opt{$oi}"];
			$optArr = explode(";", $opt);
            $newAmt += intval($optArr[1]);
            /* 포인트는 변화가 없음
			if(count($optArr) >= 3)
                $newPoint += intval($optArr[2]);
            */
        }
        //선택옵션가격
        $newAmt += (intval($row["it_option_amount"]) * $row["ct_qty"]);
		sql_query("UPDATE $g4[yc4_cart_table] SET ct_amount='{$newAmt}', ct_point='{$newPoint}' WHERE ct_id = '{$row["ct_id"]}' ");
	}
...
// 상품 이미지를 얻는다 (상품이미지, 분류이미지를 구분해서 가져옵니다
function get_it_image2($it_id, $img_type, $width=0, $height=0, $noimg=true, $id="")
{
    global $g4;

    $row = sql_fetch(" SELECT * FROM $g4[yc4_item_table] where it_id = '$it_id' ");
  
    //상품에 설정된 이미지 사용
    $str = get_it_image($it_id.$img_type, $width, $height);
    return $str;
...
function get_it_stock_qty($it_id, $ct_id="", $is_direct = false)
{
    global $g4;
    ...
    if(!$is_direct){
        ...
        $ctidQuery = "";
        if($ct_id != "") $ctidQuery = " AND ct_id != '{$ct_id}' ";
        $sql = "
            SELECT SUM(ct_qty) as sum_qty FROM $g4[yc4_cart_table] WHERE it_id = '$it_id' AND ct_status='쇼핑' AND on_uid = '$tmp_on_uid'  {$ctidQuery}
        ";
        $row = sql_fetch($sql);
        $cart = (int)$row['sum_qty'];
    }
    

    return $jaego - $daegi - $cart;
}
...
// 출력유형, 스킨파일, 1라인이미지수, 총라인수, 이미지폭, 이미지높이
// 1.02.01 $ca_id 추가
function display_mtype($type, $skin_file, $list_mod, $list_row, $img_width, $img_height, $ca_id="")
{
	global $member, $g4;
...
function display_m_itemlist($result, $skin) {
    
    global $g4;

    $file = "{$g4[path]}/m/shop/$skin";
...
function get_item_options($subject, $option, $index)
{
    ...
    {
        $str = "<select class='slt_add_option it_opt{$index}' name=it_opt{$index}[] onchange='amount_change()'>\n";
        ...
        $str .= "</select>\n<input type=hidden class='it_opt{$index}_subject' name=it_opt{$index}_subject[] value='$subject'>\n";
    }
...
function print_item_options()
{
    ...
    for ($i=1; $i<=6; $i++) 
    {
        ...
        $it_name .= "<li>".$str_split;
        $it_opt_subject = $it["it_opt{$i}_subject"];
        $opt = explode( ";", $it_opt );
        $it_name .= "<span class='cart_item_option_name'>$it_opt_subject</span> $opt[0]";
    ...
    if($it_name != "") $it_name = "<hr class='cart_item_option_hr'/><ul class='cart_item_option'>{$it_name}</ul>";
    return $it_name;
}
...
// 인수는 $it_id, $it_opt1, ..., $it_opt6 까지 넘어옴
function print_item_options_excel()
{
    global $g4;

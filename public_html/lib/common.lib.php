...
function strcut_utf8($str, $size, $isDot=true)
 {
 $pattern = "&([a-z])*;";
...
function img_thum($i_path, $w=130, $h=130){
	...
	if (file_exists($i_path)) {
		...
		$r .= "<img src='$i_path?".date("YmdHis")."' width='$width_new' height='$height_new' style='display:block;' class='simg_viewer' >";
		$r .= "<img src='/img/icon_viewer.png' border=0 align=absmiddle onclick=\"window.open('$i_path', '', 'menubar=no, toolbar=no, directories=no, location=no,status=no, top=20, left=20, width=$width, height=$height' )\" style='position:absolute;right:0px;bottom:0px;cursor:pointer'>";
...
function get_paging3($write_pages, $cur_page, $total_page, $url, $add="")
{
    ...
    if ($total_page > 1) {
        for ($k=$start_page;$k<=$end_page;$k++) {
            if ($cur_page != $k)
                $str .= " <a href='$url$k{$add}' class='page_text' >$k</a>";
            else
                $str .= " <a href='$url$k{$add}' class='page_text now_on_page' >$k</a> ";
        }
    }
...
function set_cookie($cookie_name, $value, $expire)
{
    ...
    if(isset($_SERVER['HTTPS'])){
	    setcookie(md5($cookie_name), base64_encode($value), $g4[server_time] + $expire, '/; SameSite=None; Secure;', $g4[cookie_domain]);
	} else {
...
{
    return strcut_utf8(get_text($subject), $len, $suffix);
}
...
function is_admin($mb_id)
{
    ...
    $bo_admins = explode(",", str_replace(" ", "", $board["bo_admin"]));
    if(in_array($mb_id, $bo_admins)) return 'board';
...
//간단한 XSS 체크 (전체쿼리 말고, 단위 별 검사, 파라미터 검사)
function sql_xss_check($q) {
	if(stripos($q, "select ") !== false && stripos($q, "from ") !== false) {return true;} //SELECT ~ FROM 이 들어가있으면 안됨
	if(stripos($q, "drop ") !== false && stripos($q, "table ") !== false) {return true;} //drop ~ table 이 들어가있으면 안됨
	if(stripos($q, "drop ") !== false && stripos($q, "database ") !== false) {return true;} //drop ~ database 이 들어가있으면 안됨
	if(stripos($q, "drop ") !== false && stripos($q, "column ") !== false) {return true;} //drop ~ column 이 들어가있으면 안됨
	if(stripos($q, "update ") !== false && stripos($q, "set ") !== false) {return true;} //update ~ set 이 들어가있으면 안됨
	if(stripos($q, "insert ") !== false && stripos($q, "into ") !== false) {return true;} //insert ~ into 이 들어가있으면 안됨
	if(stripos($q, "insert ") !== false && stripos($q, "values(") !== false) {return true;} //insert ~ values 이 들어가있으면 안됨
	if(stripos($q, "bulk ") !== false && stripos($q, "insert ") !== false) {return true;} //bulk ~ insert 이 들어가있으면 안됨
	if(stripos($q, "into ") !== false && stripos($q, "outfile ") !== false) {return true;} //into ~ outfile 이 들어가있으면 안됨
	if(stripos($q, "load ") !== false && stripos($q, "data ") !== false) {return true;} //load ~ data 이 들어가있으면 안됨
	return false;
}
...
{
    $sql = str_ireplace("sleep(", "sleep（", $sql);
    $sql = str_ireplace("benchmark(", "benchmark（", $sql);
    $sql = str_ireplace("information_schema.", "ⓘnformation_schema.", $sql);

...
<style>
.cartsub_icon_area { text-align:right; padding:5px 20px; font-size:14px; }
.cartsub_icon { font-size:16px; }
...
<tr>
    <td bgcolor=#FAFAFA class="" style='padding-left:10px; '>
        <div class="" style="font-size:15px; background-color:white;">
...
<tr>
    <td class="od_group" style='background-color:#ffffff;padding-left:10px;' colspan="2">
	...
	?>
		...
		<tr>
			<td class="od_group" style='padding-left:10px; background-color:#ffffff;' colspan="2">
				...
				<div class="od_group_title">
                    <span class="label_gray" style="background:white;"> <i class="fas fa-boxes"></i> 배송지 <?=$cnt?></span> 배송수량
				</div>
    ...
    else if ($default[de_card_use] && $tot_cancel_amount == 0)
    {
        ...
        if ((int)$member[mb_point] >= $od[od_temp_point]) {
            ...
            if($default[de_card_pg] != "kcp"){
             ...
             echo '<span class="btnOK" onclick="OpenWindow()">&nbsp;&nbsp;&nbsp;결제하기 <i class="fas fa-credit-card"></i>&nbsp;&nbsp;&nbsp;</span>';
            }
        } else {
...
{ 
    echo "<table cellpadding=4 cellspacing=0 width=100% class='od_group_table'>";
    echo "<colgroup width=120><colgroup width=''>";
    echo "<tr><td>· 포인트결제</td><td>: " . display_point($od[od_receipt_point]) . "</td></tr>";
    echo "</table>";
//} else if ($od[od_temp_point] > 0) {
} else if ($od[od_temp_point] > 0 && $member[mb_point] >= $od[od_temp_point]) {
    echo "<table cellpadding=4 cellspacing=0 width=100% class='od_group_table'>";
    echo "<colgroup width=120><colgroup width=''>";
    echo "<tr><td>· 포인트결제</td><td>: " . display_point($od[od_temp_point]) . "</td></tr>";
    echo "</table>";
}
...
if ($tot_cancel_amount == 0) {
    if (($od[od_temp_bank] > 0 && $od[od_receipt_bank] == 0) ||
        ...
        echo "<colgroup width=120><colgroup width=''>";
        echo "<tr><td>· 주문취소: <a href='javascript:;' onclick=\"document.getElementById('_ordercancel').style.display='block';\"><span class='btn1-o'>위의 주문을 취소합니다.</span></a></td></tr>";
...
<tr>
    <td colspan=2 align=right bgcolor='#FFF' height=70 style='font-size:15px; padding:15px; background-color:#FFFFFF;'>

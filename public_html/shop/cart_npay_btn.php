...
<?if( (!$npay_istest || ($member["mb_id"]==$default["de_npay_testid"] || $member["mb_id"]=="itmaster")) && $cartNum>0){?>
	<?if($npay_istest){?>
	...
	function buy_nc(url)
	{
		jQuery.ajax({
			type:'post',
			url:"./_ajax.naverPay.php",
			data:{on_uid:"<?=$s_on_uid?>"},
	...
	naver.NaverPayButton.apply({
		...
		BUY_BUTTON_LINK_URL:"./naverPaycart.php",
		"":""
	});
	</script>
<?}?>

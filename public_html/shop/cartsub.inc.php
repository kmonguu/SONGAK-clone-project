...
if($config["cf_use_point"])	$colspan++;
?>
...
<? if ($s_page == 'cart.php' || $s_page == 'orderinquiryview.php') echo '<colgroup width=50>'; ?>
<tr><td colspan='<?=$colspan?>' height=2 class=c1></td></tr>
<tr align=center height=28 class=c2 bgcolor="#efefef">
    <td colspan=2>상품명</td>
    <td>수량</td>
    <td>판매가</td>
    <td>소계</td>
...
for ($i=0; $row=mysql_fetch_array($result); $i++)
{
    ...
    if ($s_page == "cart.php" || $s_page == "orderinquiryview.php") { // 링크를 붙이고
        ...
        $a2 = "</a>";
        $image = get_it_image($row[it_id]."_l1", 75, 75, $row[it_id]);

    } else { // 붙이지 않고
        $a1 = "";
        $a2 = "";
        $image = get_it_image($row[it_id]."_l1", 75, 75);
    }
    ...
    if($s_page == "cart.php") { //장바구니 옵션/수량 변경 버튼
        echo "<span class='btn1-o' style='display:block; width:75px; text-align:center; font-size:11px; padding:3px 9px; margin-top:8px;' onclick='open_fixed_layer(\"{$g4["shop_path"]}/item_popup.php?it_id={$row[it_id]}&ct_id={$row[ct_id]}\", 650)'>옵션/수량변경</span>";
    }
    echo "</td>";
    ...
    else if ($s_page == "orderinquiryview.php")
    {
        switch($row[ct_status])
        {
            ...
            case '준비' : $icon = "<span class=\"cartsub_icon cartsub_icon2\" title=\"준비중\" ><i class=\"fas fa-gift\"></i></span>"; break;
            case '배송' : $icon = "<span class=\"cartsub_icon cartsub_icon3\" title=\"배송중\" ><i class=\"fas fa-truck\"></i></span>"; break;
...
<?if($default["de_npay_use"]){ //네이버페이?>
    ...
    <div class="npaybtn">
        <?include_once("{$g4["shop_path"]}/cart_npay_btn.php")?>
    </div>
<?}?>
...
<table align=center cellpadding=0 cellspacing=0 class="cartsub">
    ...
    <tr>
    	<td style='line-height:180%;border-top:1px dashed gray;border-bottom:1px dashed gray;'>
		   <div class="cartsubtext">
			<FONT COLOR="#4b4b4b"> · 상품 주문하기</FONT> : 주문서를 작성하시려면 '주문하기' 버튼을 누르세요.<BR>
			<FONT COLOR="#4b4b4b"> · 상품 수량변경</FONT> : 주문수량을 변경하시려면 원하시는 수량을 입력하신 후 '수량변경' 버튼을 누르세요.<BR>
			<FONT COLOR="#4b4b4b"> · 상품 삭제하기</FONT> : 모든 주문내용을 삭제하시려면 '삭제하기' 버튼을 누르세요.<BR>
			<FONT COLOR="#4b4b4b"> · 쇼핑 계속하기</FONT> : 쇼핑하시던 페이지로 돌아가시려면 '쇼핑 계속하기' 버튼을 누르세요.
		   </div>
        </td>
    </tr>
</table>

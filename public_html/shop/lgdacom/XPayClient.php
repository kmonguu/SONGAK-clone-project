...
class XPayClient
{
	...
	function XPayClient($home_dir, $mode="real", $MERTKEY="")
	{
	...
	function Init_TX($MID)
	{
		...
		if ($this->mertkey=="") //$this->config[$MID] == null) 
		{
	...
	{
		$auth_code = sha1($tx_id . $this->mertkey);// $this->config[$MID]);
		return $auth_code;
	}
	...
	{
		$LGD_HASHDATA = "";
		$LGD_HASHDATA = md5($LGD_MID.$LGD_OID.$LGD_AMOUNT.$LGD_TIMESTAMP.$this->mertkey);
	...
	function GetHashDataOpenpay($LGD_VENDERNO,$LGD_MID,$LGD_OPENPAY_MER_UID,$LGD_OPENPAY_TOKEN,$LGD_TIMESTAMP)
	{
		...
		if(empty($LGD_OPENPAY_TOKEN)) $InputData = $InputData . $LGD_TIMESTAMP .$this->mertkey;
		else $InputData = $InputData . $LGD_OPENPAY_TOKEN . $LGD_TIMESTAMP .$this->mertkey;
	...
	{
	    $LGD_HASHDATA = "";
	    $LGD_HASHDATA = md5($LGD_MID.$LGD_OID.$LGD_AMOUNT.$LGD_RESPCODE.$LGD_TIMESTAMP.$this->mertkey);
	    return $LGD_HASHDATA;
	}

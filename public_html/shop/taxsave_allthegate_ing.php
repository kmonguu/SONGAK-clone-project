           ...
           **************************************************************/
			...
			****************************************************************************/
			...
			if( strlen( $sRecvMsg ) == $sRecvLen )
			{
                ...
                // 올더게이트용 현금영수증 필드생성    
                //$sql = " ALTER TABLE `$g4[yc4_order_table]` ADD `od_cash_allthegate_dealno` VARCHAR( 255 ) NOT NULL "; sql_query($sql, false);
                $sql = " ALTER TABLE `$g4[yc4_order_table]` ADD `od_cash_allthegate_gubun_cd` VARCHAR( 255 ) NOT NULL , 
                                                            ADD `od_cash_allthegate_confirm_no` VARCHAR( 255 ) NOT NULL ,
                                                            ADD `od_cash_allthegate_adm_no` VARCHAR( 255 ) NOT NULL ";
                sql_query($sql, false);
                ...
                $sql = " update $g4[yc4_order_table] set od_cash_allthegate_gubun_cd = '$Gubun_cd', od_cash_allthegate_confirm_no = '$Confirm_no', od_cash_allthegate_adm_no = '$rAdm_no', od_cash = '1' where od_id = '$od_id' and on_uid = '$on_uid' ";
...
<form name=cash_pay method=post action="taxsave_allthegate_receipt.php">

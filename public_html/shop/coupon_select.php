...
<div class="Boradnlist Listlink" style="float:left;margin-top:10px; padding:10px 10px 10px 10px;">
	...
	<table cellspacing="0" cellpadding="0" class="b7" summary="주문내역 입니다." style='width:814px;'>
		<colgroup>
			<col />
			<col width="120px"/>
			<col width="200px"/>
		...
		<tbody>
			...
			<tr>
				...
				<td class="<?=$cls?>">
						<select name='coupon' class="coupons" style='height:32px; width:200px;' onchange="change_coupon(this.value, '<?=$it[it_id]?>', '<?=$row[ct_id]?>')">
...
<div class="Boradnlist Listlink" style="float:left;margin-top:10px; padding:10px 10px 10px 10px;">
	...
	<table cellspacing="0" cellpadding="0" class="b7" summary="주문내역 입니다." style='width:814px; table-layout:fixed;'>
		<colgroup>
			<col width="90px" />
			<col width="" />
			<col width="120px" />
...
<script type="text/javascript">
	...
	function change_coupon(cpn_id, it_id, ct_id){
		...
		if(cpn_id == ""){
			...
			$.post("./_ajax_cpn_amount.php", "it_id="+it_id + "&cpn_id="+cpn_id + "&ct_id="+ct_id, function(data){

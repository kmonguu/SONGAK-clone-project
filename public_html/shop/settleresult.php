...
<style>
.settle_result_table tr td {font-size:18px; font-family:Nanum Gothic 맑은고딕 굴림; }
.settle_result_table tr td table tr td{font-size:18px; padding:5px; font-family:Nanum Gothic 맑은고딕 굴림;} 
.cartBtn {background:#2d7dab; color:#ffffff; padding:8px 12px;line-height:23px; border:1px solid #346a89; font-size:12px; font-weight:bold;}
.cartBtn:hover {background:#2d7dab; color:#ffffff; padding:8px 12px;line-height:23px; border:1px solid #346a89; font-size:12px; font-weight:bold;}
.shop_btns a:hover {text-decoration:none;}
</style>


<div style="width:500px;  font-size:17px; font-family:Nanum Gothic 맑은고딕 굴림; margin:20px auto -20px auto;">
▶ 주문 및 결제완료
</div>
...
<tr><td bgcolor=#efefef height=40 align=center><?=$od['od_settle_case']?> 결제 내역</td></tr>
<tr>
    <td style='padding-left:10px'>
        <table cellpadding=5>
        <tr><td height=20> · 주문번호</td><td>: <?=$cd[od_id]?></td></tr>
        <? if ($od[od_settle_case] == '신용카드') { ?><tr><td width=100> · 승인번호</td><td><font color="#7f3ca2">: <b><?=$cd[cd_app_no]?></b></font></td></tr><? } ?>
        <tr><td height=20> · <?=$msg_settle_amount?></td><td>: <span class=amount><?=display_amount($settle_amount)?></span></td></tr>
        <? if ($od[od_settle_case] == '가상계좌') { ?><tr><td width=100> · 계좌번호</td><td><font color="#7f3ca2">: <b><?=$od[od_bank_account]?></b></font></td></tr><? } ?>
        <tr><td height=20> · <?=$msg_trade_time?></td><td>: <?=$cd[cd_trade_ymd]?> <?=$cd[cd_trade_hms]?></td></tr>
        </table>
    </td></tr>
<tr><td height=2 bgcolor=gray></td></tr>
</table><br><br>
...
<p align=center class="shop_btns" style="margin-bottom:40px; margin-top:21px;">
   	 <a href='<?="$g4[shop_path]/orderinquiryview.php?od_id=$od[od_id]&on_uid=$od[on_uid]";?>'><span class='cartBtn'>&nbsp;&nbsp;&nbsp;&nbsp;확인&nbsp;&nbsp;&nbsp;&nbsp;</span></a>
</p>

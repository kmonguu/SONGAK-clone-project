<?
define("__CASTLE_PHP_VERSION_BASE_DIR__" , dirname($_SERVER['DOCUMENT_ROOT'])."/public_html/castle");
...
if (!$g4['url'])
{
    if(!isset($_SERVER['HTTPS'])){ 
        ...
        $g4['url'] = 'https://' . $_SERVER['HTTP_HOST'];   
    }
...
else
{
    ...
    if ($tmp_mb_id = get_cookie("ck_mb_id"))
    {
        ...
        if ($tmp_mb_id != $config['cf_admin'])
        {
			...
			$app_key = md5("APP" . $_SERVER['HTTP_USER_AGENT'] . $row['mb_password']);
            // 쿠키에 저장된 키와 같다면
            $tmp_key = get_cookie("ck_auto");
            if ( ($tmp_key == $key || $tmp_key == $app_key) && $tmp_key )
            {
...
if ($is_admin != "super") {
    ...
    for ($i=0; $i<count($pattern); $i++) {
        ...
        if ($is_intercept_ip)
             die ("Access is blocked. (IP blocking) / {$_SERVER['REMOTE_ADDR']}");
    }
}

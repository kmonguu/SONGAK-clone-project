...
doInsertImage : function (images, paragraph) {
    ...
    for (i in images) {
        ...
        if (this.config.imgSetAttrAlt) {
            var fileName = "";
			try{
	            var lastDot = attr.origName.lastIndexOf('.'); 
				fileName = attr.origName.substring(0, lastDot);     
			} catch(ex){
				fileName = "";
			}
            image.setAttribute('alt', attr.alt || fileName);
        }
        else {
...
windowOpen : function (popupName) {
	...
	if (!(this.undefined(GB.popupWindow[popupName]))) {
        ...
        if (popupName === 'ImageUpload' && window.File && window.FileReader && window.FileList && window.Blob) {
			if(this.is_mobile())
				popup.tmpl = 'image.html5.mobile.html';
			else

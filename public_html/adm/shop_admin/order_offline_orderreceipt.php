...
<!-- 첫 팝업 -->
<div class="checkpop" style="display:none; position:fixed; top:0px; left:0px; height:100%; width:100%; background-color:black;opacity:0.5; z-index:999;">
    &nbsp;
</div>
<div class="checkpop" style="display:none; position:fixed; left:50%; margin-left:-200px; top:25%; text-align:left; padding-left:20px; padding-top:30px; font-size:17px; color: #CC0000; width:400px;background-color:white;border-radius:5px;z-index:9999; box-shadow:#444444 3px 3px 3px; padding-bottom:80px;">

    <div onclick="$('.checkpop').hide();" style="position:absolute; top:5px; right:10px; font-size:20px; cursor:pointer;">
    ...
    <br/>
    <div onclick="$('.checkpop').hide(); go_next();" style="position:absolute; bottom:0px; left:0px; height:40px; text-align:center; font-size:20px; padding-top:8px; width:100%; background-color:#ea4417; color:white; cursor:pointer; border-bottom-right-radius: 5px;border-bottom-left-radius: 5px">
...
<form name=frmorderreceipt method=post action='./order_offline_orderupdate.php' onsubmit="return frmorderreceipt_check(this)" autocomplete="off">
			...
			<td bgcolor=#FAFAFA class="od_group"  style='padding-left:10px' colspan="2">
				...
				<table cellpadding=3>
					...
					<?for($cidx = 0 ; $cidx < count($_POST["md_qty"][$cnt]); $cidx++){?>
						<tr>
                            <td colspan="" style="padding:8px 2px;"> 
...
<tr>
    <td bgcolor=#FAFAFA class="od_group"  style='padding-left:10px' colspan="2">
        ...
        if ($od_settle_case == "무통장") 
        {
            ...
            echo "<tr><td>입금자 이름</td><td><input type=text name=od_deposit_name class=ed size=10 maxlength=20 value=\"".proc_quot($od_name)."\"> (주문하시는분과 입금자가 다를 경우)</td></tr>";
            $receipt_amount = 0;
        }
...
function frmorderreceipt_check(f) 
{
    ...
    str_card = "";
    str = "<i class='fas fa-cart-arrow-down'></i> 총 결제하실 금액 <strong style='font-size:22px;'>" + number_format(f.od_settle_amount.value) + "원</strong> 중에서\n\n";
    if (typeof(f.od_temp_point) != 'undefined')
        str += "포인트 : <strong>" + number_format(f.od_temp_point.value) + "점</strong>\n\n";
    if(typeof(f.od_coupon) != 'undefined')
    ...
    if (typeof(f.od_receipt_bank) != 'undefined')
        str += "<?=$od_settle_case?> : <strong>" + number_format(f.od_receipt_bank.value) + "원</strong>\n\n";
    str += "으로 주문 하셨습니다.\n\n"+

...
$sql_common = " ca_id            = '$ca_id',
                ...
                it_maker         = '$it_maker',
                it_origin        = '$it_origin',
                it_option1_subject = '$it_option1_subject',
				it_option1 = '$it_option1',
				it_option2_subject = '$it_option2_subject',
				it_option2 = '$it_option2',
				it_option3_subject = '$it_option3_subject',
...
if ($w == "" || $w == "u") {
	...
	$optObj->delete_item($it_id);
	$options = Common::reArrayRequest("io_type1");
	if(count($options) > 0){
		foreach($options as $opts){
			//echo $opts["io_type1"].$opts["io_type2"].$opts["io_type3"].$opts["io_amt"];
			//$o = $optObj->get_option($it_id, $opts["io_type1"], $opts["io_type2"], $opts["io_type3"]);
			/*
			if($o["no"]){
				$optObj->update($opts);
			} else {

		...
		<table cellpadding="0" cellspacing="0" border="0">
			<tr>
				<td>
					...
					<script>
						$("#sel_field > option[value='<?=$sel_field?>']").attr("selected", "selected");
					</script>
				</td>
				...
				<td style="padding:0 0 0 5px;">
					<input type="button" class="btn1" value=" 엑셀저장 " onclick="print_excel(document.f_excel,'delivery');"> * 운송장번호 입력용 엑셀 폼으로 저장
				</td>
			</tr>
		</table>
	</td>
	 <td width=30% align=right style="padding:0 5px 0 0;">
		<table cellpadding="0" cellspacing="0" border="0">
			<tr>
				<td>
...
for ($i=0; $row=sql_fetch_array($result); $i++) {
    ...
    if($row["it_option1"]) {
        $it_name .= $optObj->print_option_cart($row["it_id"], $row["it_option1"], $row["it_option2"], $row["it_option3"], $row["it_option_amount"])."<br/>";
    }
    ...
    <tr class='list$list center'>
        ...
        <td align=left style='padding-left:5px; text-align:left;'>$it_name</td>
...
function print_excel(){
	...
	f.action = "./orderstatuslist_excel.php";
	f.submit();
	f.action = tmpAction;
}
</script>

...
<tr align=center class='bgcol1 bold col1 ht center'>
    ...
    <td><?=subject_sort_link("it_order", "sca=$sca")?>순서</a></td>
    <td><?=subject_sort_link("it_use", "sca=$sca", 1)?>판매</a></td>
    <td><?=subject_sort_link("it_hit", "sca=$sca", 1)?>조회</a></td>
    <td><a href='./itemform.php'><img src='<?=$g4[admin_path]?>/img/icon_insert.gif' border=0 title='상품등록'></a></td>
</tr>
...
for ($i=0; $row=mysql_fetch_array($result); $i++)
{
    ...
    <tr class='list$list'>
        ...
        <td rowspan='2' align=left>$tmp_ca_list&nbsp;<input type='text' name='it_name[$i]' value='".htmlspecialchars2(cut_str($row[it_name],250, ""))."' required size=40 class=ed></td>
		...
		<td rowspan='2'><input type='text' name='it_order[$i]' value='$row[it_order]' class=ed size=3 style='text-align:right;'></td>
        <td rowspan='2'><input type=checkbox name='it_use[$i]' ".($row[it_use] ? "checked" : "")." value='1'></td>
        <td rowspan='2'>$row[it_hit]</td>
        <td rowspan='2'>$s_mod $s_del $s_vie $s_copy</td>
    </tr>
	<tr class='list$list'>

    ...
    <td width=50% align=left style="padding:0 0 0 5px;">
		<table cellpadding="0" cellspacing="0" border="0">
			<tr>
				...
				<td style="padding:0 0 0 5px;"> <input type=hidden name=save_search value='<?=$search?>'><input type=text name=search value='<? echo $search ?>' autocomplete="off"></td>
...
for ($i=0; $row=mysql_fetch_array($result); $i++) 
{
    ...
    if ($i>0)
        echo "<tr><td colspan=12 height=1 bgcolor='#000' style='font-size:1px; padding:0;border:1px solid black;'></td></tr>";
	...
	for ($k=0; $row2=sql_fetch_array($result2); $k++) {
		array_push($itemlist, $row2);
	}
	
	$rowspan = count($itemlist) * 2 + 1;

    $list = $i%2;
    echo "
    <tr class='list$list ht'>
        <td rowspan='{$rowspan}' align=center title='주문일시 : $row[od_time]'><a href='$g4[shop_path]/orderinquiryview.php?od_id=$row[od_id]&on_uid=$row[on_uid]'>$row[od_id]</a></td>
        <td rowspan='{$rowspan}'><a href='$_SERVER[PHP_SELF]?sort1=$sort1&sort2=$sort2&sel_field=od_name&search=$row[od_name]'><span title='$od_deposit_name'>".$row[od_name]."</span></a></td>
        ...
        <td rowspan='{$rowspan}' align=center>$s_mod $s_del</a></td>
    </tr>";
    ...
    for ($k=0; $k < count($itemlist); $k++) 
    {
		...
		if($row2["it_option1"]) {
			$it_name .= $optObj->print_option_cart($row2["it_id"], $row2["it_option1"], $row2["it_option2"], $row2["it_option3"], $row2["it_option_amount"])."<br/>";
		}
        ...
        <tr class='list$list ht'>
            <td colspan=3>
                <table cellpadding=0 cellspacing=0 style='table-layout:fixed; width:100%; max-width:400px; margin:0; border:0px;'>
				<colgroup>
					<col width='100px' />
					<col/>
				</colgroup>
                <tr>
                	<td style='padding-top:5px; padding-bottom:5px; border:0px;'><a href='$href'>".get_it_image("{$row2[it_id]}_s", 50, 50)."</a></td>
                	<td style='border:0px; text-align:left;'>$it_name</td>
                </tr>
                </table></td>

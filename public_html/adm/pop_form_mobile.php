...
<form name=oneform method=post action="./pop_form_mobileUpdate.php" enctype="multipart/form-data" onsubmit='return foneform_submit(this);' style="margin:0px;">
...
<tr class=ht>
	<td class="head">팝업사용</td>
     <td colspan="3">
		<select name='pop_type' style='width:85px;'>
			<option value='Y' <?=($ca[pop_type] == "Y" ? "selected" : "")?>>사용</option>
...
<tr class=ht style="display:none;">
	...
	<td colspan="3">
		<input type=radio name=pop_img_size value="1" > 원본크기 &nbsp;
		<input type=radio name=pop_img_size value="2" > 설정값 &nbsp;
		<input type=radio name=pop_img_size value="3" checked> 팝업창 크기에 맞춤 &nbsp;
	</td>
</tr>
...
<p align=center style="width:1000px;margin:10px 0 0 0;">
    ...
    <input type=button class=btn1 accesskey='l' value='  목  록  ' onclick="document.location.href='./pop_list_mobile.php?<?=$qstr?>';">
<?}?>
</form>
...
function foneform_submit(f) {
    ...
    f.action = "./pop_form_mobileUpdate.php";
    return true;
}

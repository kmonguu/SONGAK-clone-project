    ...
    <td width=50% align=left style="padding:0 0 0 5px;">
		<table cellpadding="0" cellspacing="0" border="0">
			<tr>
				...
				<td style="padding:0 0 0 5px;"><input type=text name=stx class=ed required itemname='검색어' value='<?=$stx?>'></td>
...
<tr class='bgcol1 bold col1 ht center'>
    ...
    <td><?=subject_sort_link("bo_subject")?>제목</a> <?=subject_sort_link("a.gr_id")?>그룹</a> <?=subject_sort_link("bo_skin", "", "desc")?>스킨</a></td>
    <td title="글읽기 포인트"><?=subject_sort_link("bo_read_point")?>읽기포인트</a></td>
    <td title="글쓰기 포인트"><?=subject_sort_link("bo_write_point")?>쓰기포인트</a></td>
    <td title="코멘트쓰기 포인트"><?=subject_sort_link("bo_comment_point")?>코멘트포인트</a></td>
    <td title="다운로드 포인트"><?=subject_sort_link("bo_download_point")?>다운포인트</a></td>
    <td title="검색사용"><?=subject_sort_link("bo_use_search")?>검색사용</a></td>
	<?if($exPushApp){?>
		<td title="PUSH"><?=subject_sort_link("bo_use_pushmsg")?>푸시메시지사용</a></td>
	<?}?>
    <td title="검색순서"><?=subject_sort_link("bo_order_search")?>검색순서</a></td>
	<td><a href="./board_form.php"><img src='<?=$g4[admin_path]?>/img/icon_insert.gif' border=0 title='생성'></a></td>
</tr>
<?
// 스킨디렉토리
$skin_options = "";
...
for ($i=0; $row=sql_fetch_array($result); $i++) {
    ...
    */
    ...
    echo "<td align=left height=25>".get_group_select("gr_id[$i]", $row[gr_id])."&nbsp;<input type=text class=ed name=bo_subject[$i] value='".get_text($row[bo_subject])."' style='width:100px'>&nbsp;<select id=bo_skin_$i name=bo_skin[$i]>$skin_options</select></td>";
    ...
    echo "<td title='검색사용'><input type=checkbox name=bo_use_search[$i] ".($row[bo_use_search]?'checked':'')." value='1'></td>";

	if($exPushApp){
		echo "<td title='푸시메시지'><input type=checkbox name=bo_use_pushmsg[$i] ".($row[bo_use_pushmsg]?'checked':'')." value='1'></td>";
	}

    echo "<td title='검색순서'><input type=text class=ed name=bo_order_search[$i] value='$row[bo_order_search]' size=2></td>";
...
function board_copy(bo_table) {
    window.open("./board_copy.php?bo_table="+bo_table, "BoardCopy", "left=10,top=10,width=500,height=290");
}
</script>
...
$(function(){
	$(".list tr input").change(function(){
		$(this).closest("tr").find("td:first").find("input").prop("checked", "checked");
	});
});
</script>

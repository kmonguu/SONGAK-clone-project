...
function member_delete($mb_id)
{
    ...
    $sql = " select mb_name, mb_nick, mb_ip, mb_recommend, mb_memo, mb_level, mb_is_naver, mb_is_kakao, mb_is_facebook from $g4[member_table] where mb_id= '$mb_id' ";
    $mb = sql_fetch($sql);
    if ($mb[mb_recommend]) {
    ...
    if($mb["mb_is_naver"] || $mb["mb_is_kakao"] || $mb["mb_is_facebook"]){  //API  ID로 회원가입일경우 완전 삭제
...
function icon($act, $link="", $target="_parent")
{
    ...
    $icon = "<img src='{$g4[admin_path]}/img/icon_{$img[$act]}.gif' border=0 align=absmiddle title='$act' >";
    if ($link)
...
{
    //admin.tail.php에서 tooltipsy.js를 사용하여 출력
    global $g4;
    static $idx = 0;
    ...
    //$id = md5(date("YmdHis")+rand(0, 9999));
    
    $str  = "<img class='tooltip' title='{$help}' src='$g4[admin_path]/img/icon_help.gif' border=0 width=15 height=15 align=absmiddle style='cursor:hand;'>";
    /*
...
function subtitle($title, $more="") 
{
    ...
    $s = "<table width=100% cellpadding=0 cellspacing=0><tr><td width=80% align=left><table border='0' cellpadding='0' cellspacing='1'><tr><td height='24'><img src='$g4[admin_path]/img/icon_title.gif' > <font color='#525252'><b>$title</b></font> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td></tr></table><table width=100% cellpadding=0 cellspacing=0><tr><td height=1></td></tr></table></td><td width=20% align=right>";
    if ($more)
...
if (get_session("ss_mb_key") !== $admin_key) {
	...
	alert("접속환경(IP,UA)의 변화가 감지되었습니다, 다시 로그인 해주시기 바랍니다.", "$g4[bbs_path]/logout.php");

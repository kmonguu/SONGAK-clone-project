...
<form name=fcategorylist method='post' action='./pop_list_mobileUpdate.php' autocomplete='off' style="margin:0px;">
...
<tr align=center class='bgcol1 bold col1 ht center'>
	...
	<td width=''><a href='./pop_form_mobile.php'><img src='<?=$g4[admin_path]?>/img/icon_insert.gif' border=0 title='골프등록'></a></td>
</tr>
...
for ($i=0; $row=sql_fetch_array($result); $i++)
{
    ...
    $s_add = icon("추가", "./pop_form_mobile.php?no=$row[pop_no]&$qstr");
    $s_upd = icon("수정", "./pop_form_mobile.php?w=u&no=$row[pop_no]&$qstr");
    //$s_vie = icon("보기", "$g4[shop_path]/list.php?no=$row[no]");

    if ($is_admin == 'super')
	$s_del = icon("삭제", "javascript:del('./pop_form_mobileUpdate.php?w=d&no=$row[pop_no]&$qstr');");
    ...
    <tr class='list$list center ht'>
		...
		<td align='center'>
			<select name='pop_type[$i]' style='width:85px;'>
				<option value='Y' ".($row[pop_type] == "Y" ? "selected" : "").">사용</option>

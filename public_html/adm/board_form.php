...
<colgroup width=6% class='left'>
<colgroup width=20% class='col1 pad1 bold right'>
<colgroup width=74% class='col2 pad2'>
<tr>
...
<tr class='ht'>
    ...
    <td class="head">게시판 관리자</td>
    <td><input type=text class=ed name=bo_admin maxlength=255 value='<?=$board[bo_admin]?>'></td>
</tr>

<tr class='ht'>
    <td><input type=checkbox name=chk_list_level value=1></td>
    <td class="head">목록보기 권한</td>
    <td>
...
<tr class='ht'>
    ...
    <td class="head">전체목록보이기 사용</td>
    <td><input type=checkbox name=bo_use_list_view value='1' <?=$board[bo_use_list_view]?'checked':'';?>>사용</td>
</tr>


<?
$appPath = $g4[path].$g4['app_path'];
if(is_file($appPath."/index.php")){
?>
<tr class='ht'>
    <td><input type=checkbox name=bo_use_pushmsg value=1></td>
    <td class="head">푸시메시지 사용</td>
    <td><input type=checkbox name=bo_use_pushmsg value='1' <?=$board[bo_use_pushmsg]?'checked':'';?>>사용
    &nbsp;<?=help("새로운 글 등록시 홈페이지관리 어플리케이션을 통해서 푸시메시지를 전송합니다.")?>
    </td>
</tr>
<?}?>
...
<tr class='ht'>
    ...
    <td>
        <select name=bo_sort_field>
			...
			<option value='wr_10 asc, wr_num, wr_reply'>wr_10 : 관리자 지정 내림차순
        </select>

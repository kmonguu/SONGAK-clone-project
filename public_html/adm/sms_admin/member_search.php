...
if($sch_syear){
	$sql_search .= " AND mb_birth >= '{$sch_syear}0101' ";
}

if($sch_eyear){
	$sql_search .= " AND mb_birth <= '{$sch_eyear}1231' ";
}
		...
		<table cellpadding="0" cellspacing="0" border="0">
			<tr>
				...
				<td style="padding:0 0 0 5px;">
					<input type=text id="add_name" class=ed style="width:100px;">
				</td>
				<td style="padding:0 0 0 10px;">
					핸드폰 번호 : 
				</td>
				<td style="padding:0 0 0 5px;">
					<input type=text id="add_hp" class="ed phonenum" style="width:100px;">
				</td>
				
				<td style="padding:0 0 0 10px;">
					<input type='button' value='추가' onclick='add_member()'/>
				</td>
...
<tr>
    <td width=100% align=left style="padding:0 0 0 5px;">
		<table cellpadding="0" cellspacing="0" border="0">
			<tr>
				<td>
					<select name=sfl class=cssfl style="width:50px;">
				...
				<td style="padding:0 0 0 5px;"><input type=text name=stx class=ed  itemname='검색어' value='<? echo $stx ?>' style="width:70px;"></td>
				...
				<?
					...
					$ageE = "";
					$year = date("Y");
					if($sch_syear) $ageS = ($year - $sch_syear + 1)."세";
					if($sch_eyear) $ageE = ($year - $sch_eyear + 1)."세";
				?>
				<td>
					&nbsp;&nbsp;&nbsp;▶&nbsp; 출생년도 :
					<input type="text" style="text-align:right; width:40px;" name="sch_syear" value="<?=$sch_syear?>" onchange="ck_age(this.value, 's')"/>년
					~
					<input type="text" style="text-align:right; width:40px;" name="sch_eyear" value="<?=$sch_eyear?>"  onchange="ck_age(this.value, 'e')"/>년
					&nbsp;
					(<span class="disp_age_e"><?=$ageE?></span> ~ <span class="disp_age_s"><?=$ageS?></span>)
				</td>
				<td style="padding:0 0 0 5px;"><input type=image src='<?=$g4[admin_path]?>/img/btn_search.gif' align=absmiddle></td>
				
				<script>
						function ck_age(ck, div) { 
							if(ck == ""){
							   $(".disp_age_"+div).html("");
							   return;
							}
...
function add_member() {
	var mb_id = "";
	var mb_name = $("#add_name").val();
	var mb_hp = $("#add_hp").val();
	...
	top.select_member(mb_id, mb_name, mb_hp);
}
</script>

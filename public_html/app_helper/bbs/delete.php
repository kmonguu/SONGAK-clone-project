...
// 4.11
@include_once("$board_skin_mpath/delete_all.head.skin.php");
...
// 사용자 코드 실행
@include_once("$board_skin_mpath/delete_all.skin.php");
...
for ($i=count($tmp_array)-1; $i>=0; $i--) 
{
	...
	echo $write[mb_id];
		echo "<br/>";
	echo sql_password($wr_password);
		echo "<br/>";
	echo $write[wr_password];
	exit;
	*/

	//$wr_password && !$write[mb_id] && sql_password($wr_password) == $write[wr_password]
    ...
    // 원글만 구한다.
	/* 답글 존재할 경우 삭제 안함 => 관리자는 (관리자 어플)은 답글존재 상관없이 삭제
	...
	*/
    ...
    //푸시메시지에서 삭제
    sql_query(" delete from it9_gcm_msg where bo_table='$bo_table' and wr_id='$write[wr_id]' ");
...
// 4.11
@include_once("$board_skin_mpath/delete_all.tail.skin.php");

//goto_url("./board.php?bo_table=$bo_table&page=$page" . $qstr);
goto_url($g4["mpath"]."/pages.php?p={$p}");
?>

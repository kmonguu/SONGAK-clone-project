...
.view_contents, .view_contents div, .view_contents p, .view_contents span {font-size:24px !important; line-height:1.5 !important; }
</style>
...
<div style="height:28px; clear:both;"></div>
	...
	for ($i=0; $i<count($view[file]); $i++) {
		if ($view[file][$i][source] && !$view[file][$i][view]) {
			...
			$link=str_replace("./", "http://jbl.or.kr/app/bbs/",$view[file][$i][href]);
	...
	?>
	<div class="view_contents" style="max-width:640px;">
		<?=$view['content']?>
	</div>
	<div class="blank"></div>
...
function gotFileEntry(fileEntry) {
    ...
    var documentUrl = dlink;
    var uri = encodeURI(documentUrl);
    fileTransfer.download(uri, cdr.nativeURL + dfile,
        function(entry) {

            alert_app("/Download 폴더에 다운로드가 완료되었습니다. ");
			closeProgress();
        },
        function(error) {

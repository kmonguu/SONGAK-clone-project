...
<script>
var member_skin_path = "<?=$member_skin_mpath?>";
</script>
<script type="text/javascript" src="<?=$member_skin_mpath?>/ajax_register_form.jquery.js"></script>
...
<tr>
    <td style="height:30px"><!-- <img src="<?=$member_skin_mpath?>/img/join_form_title.gif" width="624" height="72"> -->
	</td>
...
<tr>
    <td bgcolor="#CCCCCC">
        ...
        <TR bgcolor="#FFFFFF">
            ...
            <TD class=m_padding>
                <label for="reg_mb_id"><input type="text" class=ed maxlength=20  style="height:30px;font-size:25px" size=10 id='reg_mb_id' name="mb_id" value="<?=$member[mb_id]?>" <? if ($w=='u') { echo "readonly style='background-color:#dddddd;'"; } ?>
                    <? if ($w=='') { echo "onblur='reg_mb_id_check();'"; } ?>></label>
        ...
        <TR bgcolor="#FFFFFF">
            ...
            <TD class=m_padding><label for="mb_password"><INPUT class=ed type=password name="mb_password" id="mb_password" size=10 style="height:30px;font-size:25px" maxlength=20 <?=($w=="")?"required":"";?> itemname="패스워드"></label></TD>
        </TR>
        <TR bgcolor="#FFFFFF">
            <TD class=m_title>패스워드 확인</TD>
            <TD class=m_padding><label for="mb_password_re"><INPUT class=ed type=password name="mb_password_re" size=10 style="height:30px;font-size:25px" maxlength=20 <?=($w=="")?"required":"";?> itemname="패스워드 확인"></label></TD>
        </TR>
        <!-- <TR bgcolor="#FFFFFF">
...
<tr>
    <td bgcolor="#CCCCCC">
        <TABLE cellSpacing=1 cellPadding=0 width=100%>
        <TR bgcolor="#FFFFFF">
            <TD width="160px" class=m_title>이름</TD>
            <TD class=m_padding>
                <label for="mb_name"><input name=mb_name id="mb_name" itemname="이름" size=10 style="height:30px;font-size:25px" value="<?=$member[mb_name]?>" <?=$member[mb_name]?"readonly class=ed2":"class=ed";?>></label>
        ...
        <TR bgcolor="#FFFFFF">
            ...
            <TD class='m_padding lh'>
                <label for="reg_mb_nick"><input class=ed type=text id='reg_mb_nick' size=10 style="height:30px;font-size:25px" name='mb_nick' maxlength=20 value='<?=$member[mb_nick]?>'
                    onblur="reg_mb_nick_check();"></label>
		...
		<TR bgcolor="#FFFFFF">
            <TD class=m_title>자원봉사자</TD>
            <TD class=m_padding>
				<input type="checkbox" name="mb_3" id="mb_3" value="1" <?=$member[mb_3] ? "checked" : ""?> />
				<label for="mb_3">자원봉사자</label>
				<?if($member[mb_3] == 1 && $member[mb_level] == 2){?>
					<span style="color:red;">(관리자가 자원봉사자 등급을 확인하고 있습니다.)</span>
				<?}?>
			</TD>
        </TR>
		 ...
		 <TR bgcolor="#FFFFFF">
            <TD class=m_title><label for="mb_1">장애명</label></TD>
            <TD class=m_padding>
					<select name="mb_1" id="mb_1" style="font-size: 25px;">
						<option value="없음">없음</option>
						<option value="시각장애">시각장애</option>
						<option value="지체장애">지체장애</option>
						<option value="정신장애">정신장애</option>
						<option value="독서장애">독서장애</option>
						<option value="기타">기타</option>
					</select>
					<script>
						$("#mb_1 > option[value='<?=$member[mb_1]?>']").attr("selected", "selected");
					</script> 
			</TD>
        </TR>
		 ...
		 <TR bgcolor="#FFFFFF">
            <TD class=m_title><label for="mb_2">장애등급</label></TD>
            <TD class=m_padding>
					<select name="mb_2" id="mb_2"  style="font-size: 25px;">
						<option value="없음">없음</option>
					...
					<script>
						$("#mb_2 > option[value='<?=$member[mb_2]?>']").attr("selected", "selected");
					</script>
			</TD>
        </TR>
		...
		<label for="email1"><input class=ed name="email1" id="email1" value="<?=$email[0]?>" style="font-size: 25px;width:150px;height:30px;" required itemname="이메일"> </label>
		<span style="font-size: 25px;">@ </span>
		<input style="font-size: 25px;width:150px;height:30px;" class=ed maxlength=26 size=26 name="email2" id="email2" required itemname="이메일" value="<?=$email[1]?>">

		<select id="email_selector" style="font-size: 25px;">
		...
		jQuery(window).ready(function(){
			...
			jQuery("#email_selector").bind("blur",function(){
				...
				<script type="text/javascript">
					$("#email_selector > option[value='<?=$member[mb_email]?>']").attr("selected", "selected");
				</script>

                <br /><span id='msg_mb_email'></span>
        ...
        <? if ($w=="") { ?>
            <TR bgcolor="#FFFFFF">
                ...
                <TD class=m_padding><label for="mb_birth"><input class=ed type=text id=mb_birth name='mb_birth' style="width:160px;height:30px;font-size:25px" maxlength=8 minlength=8 required numeric itemname='생년월일' value='<?=$member[mb_birth]?>' readonly title='옆의 달력 아이콘을 클릭하여 날짜를 입력하세요.'></label></TD>
            </TR>
        <? } ?>
        ...
        <? } else { ?>
            <TR bgcolor="#FFFFFF">
                ...
                <TD class=m_padding>
                    <select id=mb_sex style="font-size:22px;" name=mb_sex required itemname='성별'>
						<option value=''>선택하세요
						<option value='F'>여자
						<option value='M'>남자
                    </select>
        ...
        <? if ($config[cf_use_hp]) { ?>
         <TR bgcolor="#FFFFFF">
            ...
            <TD class=m_padding>
				<input type="hidden" class='ed' style="width:120px;" name=mb_hp id="mb_hp" itemname="핸드폰" required value="<?=$member[mb_hp]?>">
				<?
				$hp = explode("-", $member[mb_hp] ? $member[mb_hp] : $member[mb_hp]);
				$mb_hp1 = $hp[0];
				$mb_hp2 = $hp[1];
				$mb_hp3 = $hp[2];
				?>
				<input type="text" class='ed' maxlength=3 id="hp1" style="width:50px;height:30px;font-size:25px" value="<?=$mb_hp1?>" required >
				<span style="font-size:22px;height:30px;line-height:10px;"> - </span>
				<input type="text" class='ed' maxlength=4 id="hp2" style="width:70px;height:30px;font-size:25px" value="<?=$mb_hp2?>" required>
				<span style="font-size:22px;height:30px;line-height:10px;"> - </span>
				<input type="text" class='ed' maxlength=4 id="hp3" style="width:70px;height:30px;font-size:25px" value="<?=$mb_hp3?>" required>
				<!--휴대폰 입력부 -->
				<script type="text/javascript">
					jQuery("#hp1,#hp2,#hp3").bind("blur",function(){
						jQuery("#mb_hp").val( jQuery("#hp1").val() + "-"+ jQuery("#hp2").val() + "-"+ jQuery("#hp3").val());
					});
        ...
        <TR bgcolor="#FFFFFF">
            ...
            <TD valign="middle" class=m_padding>
                ...
                <tr>
                    <td height="25"><input class=ed type=text name='mb_zip1' maxlength=5 readonly <?=$config[cf_req_addr]?'required':'';?> itemname='우편번호' value='<?=$member[mb_zip1]?>' style="width:150px; height:30px;font-size:25px">
                        &nbsp;<a href="javascript:;" onclick="openDaumPostcode('fregisterform', 'mb_zip1', 'mb_addr1', 'mb_addr2');"><img width="91" height="20" src="<?=$member_skin_mpath?>/img/post_search_btn.gif" border=0 align=absmiddle></a></td>
                </tr>
                <tr>
                    <td height="25" colspan="2"><input class=ed type=text name='mb_addr1' style="width:390px; height:30px;font-size:25px" readonly <?=$config[cf_req_addr]?'required':'';?> itemname='주소' value='<?=$member[mb_addr1]?>'></td>
                </tr>
                <tr>
                    <td height="25" colspan="2"><input class=ed type=text name='mb_addr2' style="width:390px; height:30px;font-size:25px" itemname='상세주소' value='<?=$member[mb_addr2]?>'></td>
                </tr>
                </table>
            </TD>
        </TR>
...
<tr>
    <td bgcolor="#CCCCCC">
        ...
        <TR bgcolor="#FFFFFF">
            ...
            <TD class=m_padding><input type=checkbox name=mb_mailling id="mb_mailling" value='1' <?=($w=='' || $member[mb_mailling])?'checked':'';?>><label for="mb_mailling" style="font-size:15px;">정보 메일을 받겠습니다.</label></TD>
        </TR>
        <TR bgcolor="#FFFFFF">
            <TD width="160" class=m_title>SMS 수신여부</TD>
            <TD class=m_padding><input type=checkbox name=mb_sms id="mb_sms" value='1' <?=($w=='' || $member[mb_sms])?'checked':'';?>><label for="mb_sms" style="font-size:15px;">핸드폰 문자메세지를 받겠습니다.</TD>
        </TR>
        ...
        <TR bgcolor="#FFFFFF"> 
            ...
            <TD class=m_padding><input type=checkbox name=mb_open id="mb_open" value='1' <?=($w=='' || $member[mb_open])?'checked':'';?>><label for="mb_open" style="font-size:15px;">다른분들이 나의 정보를 볼 수 있도록 합니다. 
...
<tr>
    <td bgcolor="#CCCCCC">
        ...
        <TR bgcolor="#FFFFFF">
            ...
            <td class=m_padding>
                <label for="wr_key"><input type=input class=ed size=10 style="height:30px;font-size:25px" name=wr_key id="wr_key" itemname="자동등록방지" required></label>&nbsp;&nbsp;왼쪽의 글자를 입력하세요.
            </td>
        </tr>
        </table>
    </td>
...
<? if ($w=='u') {?>
<p align=center style="margin:40px 0 50px 0">
    <INPUT type=image src="<?=$member_skin_mpath?>/img/ok_btn1.jpg" border=0 accesskey='s'>
</p>

<?}else{?>
<p align=center style="margin:40px 0 50px 0">
    <INPUT type=image src="<?=$member_skin_mpath?>/img/join_ok_btn.jpg" border=0 accesskey='s'>
</p>
<?}?>

<?if($w=="u"){?>
	<a href="javascript:member_leave();">
		<span style="display:inline-block; width:100px; height:35px; line-height:35px; background:#eee; color:#222; text-align:center;">회원탈퇴</span>
	</a>
<?}?>
</td></tr>
</table>
...
function member_leave()
{
    if (confirm("정말 회원에서 탈퇴 하시겠습니까?"))
            location.href = "<?=$g4[mpath]?>/bbs/member_confirm.php?url=member_leave.php";
}
</script>

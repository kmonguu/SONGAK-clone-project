        ...
        <ul style='line-height:2;margin:10px 10px 30px 10px;'>
            <? if (!$view[wr_reply]) { ?>
                <li>
                    ...
                    <?
                    //	1:진행중 | 2:마감처리
                    $statusKo = Array("","예약접수","예약처리중","예약완료");
                    $status = preg_replace("[^1-9]","",$view[wr_10]);
                    if($status == "") $status = 1;
                    ?>
                    <span style="float:right;width:70%;line-height:2;"><?php  echo $statusKo[$status]; ?></span>
                </li>
            <?}?>

            <li><strong>연락처</strong> <span style="float:right;width:70%;line-height:2;"><a href="tel:<?=$view[wr_1]?>"><?=$view[wr_1]?></a></span></li>

...
// author : Ko gi boong
// DESC : /bbs/write_update.php, write_comment_update.php 상단
//=====================================================================================================================================
*/
if($w != "cu" && !$_REQUEST["antispam_rkey"]) {
	alert("스팸게시글로 분류되어 차단되었습니다.\\n관리자에게 문의해주십시오\\n[CODE:IT9_3000]"); 
}
$curl = curl_init();
curl_setopt_array($curl, array(
CURLOPT_RETURNTRANSFER => 1,
CURLOPT_URL => 'http://antispam.1937.co.kr/checkspam_common_v3.php',
CURLOPT_POST => 1,
...
"name"=>$_REQUEST[wr_name],
"title"=>$_REQUEST[wr_subject],
"key"=>$_REQUEST["antispam_rkey"],
"w"=>$w,
"bo_table"=>$bo_table,
"as_version"=>"3.1",
...
$chkspam_data = curl_exec($curl);
curl_close($curl);
if($chkspam_data != ""){
	$chkspam = explode("|", $chkspam_data);
	if($chkspam[0]==1){ alert($chkspam[1]);exit;} //문구검사
	if($chkspam[0]==2){ header($chkspam[1]); exit;} //해외IP검사
	if($chkspam[0]==3){ alert($chkspam[1]);exit;} //KEY 검사
};
//=====================================================================================================================================
...
for ($i=0; $i<count($_FILES[bf_file][name]); $i++)
{
	...
	if($tmp_file){
		...
		if ( preg_match("/\.($config[cf_image_extension])$/i", $filename) ||
            ...
            $tmp_value = @getimagesize($tmp_file);
			if ($tmp_value[2] < 1 || $tmp_value[2] > 16)
            {
                continue;
            }
            ...
            if($image->getWidth() > (int)$board[bo_image_width]){
                $image->resizeToWidth((int)$board[bo_image_width]);
                $image->save($tmp_file);
            }
        }
	...
	//이름이 너무 길경우 잘라서 저장시킵니다. 2015-05-21 (추가자 : 고기붕)
	$cutFilename = "";
	if(strlen($upload[$i][file]) > 200) {
		$tFn = explode(".", $upload[$i][file]);
		$tFn[0] = cut_str($tFn[0], 190);	
		foreach($tFn as $ke=>$v){
			if($ke == (count($tFn)-1)) $cutFilename.=".";
			$cutFilename .= $v;
		}
		$upload[$i][file] = $cutFilename;
	}
...
if ($w == "" || $w == "r")
{
    if ($member[mb_id])
    {
        ...
        $wr_email = $wr_email ? $wr_email : $member[mb_email];
        $wr_homepage = $wr_homepage ? $wr_homepage : $member[mb_homepage];
    }
    else
    {
        $mb_id = "";
...
else if ($w == "u")
{
    ...
    if ($member[mb_id])
    {
        ...
        else
        {
            $mb_id = $wr[mb_id];
            $wr_name = $wr[wr_name];
            $wr_email = $wr_email ? $wr_email : $wr[wr_email];
            $wr_homepage = $wr_homepage ? $wr_homepage : $wr[wr_homepage];
        }
    }
    else
    {
...
if (!($w == "u" || $w == "cu") && $config[cf_email_use] && $board[bo_use_email])
{
   ...
   if(trim($wr_email) == "")
	$wr_email = "unknown@unknown.co.kr";
...
//푸시메시지 전송 (app가 설치되어 있으면)
$appPath = $g4[path].$g4['app_path'];
if(is_file($appPath."/index.php")){
	
	if($w == "" && $board[bo_use_pushmsg]) { //푸시메시지 사용일 때
		
		@include_once($appPath."/push.php");

		$title = $board[bo_subject]." 게시글이 등록되었습니다.";
		$msg = $_POST[wr_subject];

		sendPush($title, $msg, "", $bo_table, $wr_id);
...
if(is_file("{$g4["path"]}/app_helper/app.php")) {
    if($w == "" && $board[bo_use_pushmsg]) { //푸시메시지 사용일 때
		...
		$msg = $_POST[wr_subject];
        $fcmObj = new HpFcm();
        $fcmObj->send($title, $msg, array(), $bo_table, $wr_id, "board");
    }
}

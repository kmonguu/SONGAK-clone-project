...
// author : Ko gi boong
// DESC : /bbs/register_form_update.php 상단
//=====================================================================================================================================
*/
if($w == "") {
    if(!$_REQUEST["antispam_rkey"]) {
        alert("스팸성 접근으로 판단되어 차단되었습니다.\\n관리자에게 문의해주십시오\\n[CODE:IT9_3000]"); 
    }
    $curl = curl_init();
    ...
    "content"=>$_REQUEST["mb_id"],
    "name"=>$_REQUEST["mb_name"],
    "title"=>"회원가입",
    "key"=>$_REQUEST["antispam_rkey"],
    "w"=>$w,
    "bo_table"=>"register_form",
    "as_version"=>"3.1",
...
if ($HTTP_COOKIE_VARS[7777] != "antispamreg")  alert('잘못된 경로로 접근하셨습니다. 해당 오류가 지속적으로 표시될 경우 관리자에게 문의해주세요[C]'); 
if($_SESSION["as77"] != "antispamreg") alert("잘못된 경로로 접근하셨습니다. 해당 오류가 지속적으로 표시될 경우 관리자에게 문의해주세요[S]"); 
...
$mb_dir = "$g4[path]/data/member/".md5(substr($mb_id, 0, 2));
...
{
    if (preg_match("/\.(gif|jpeg|jpg)$/i", $_FILES[mb_icon][name])) 
    {
        ...
        if ($_FILES[mb_icon][size] <= $config[cf_member_icon_size]) 
        {
			...
			$imgtype = getimagesize($_FILES[mb_icon][tmp_name]);
			$ext = $imgtype[2] ==  IMG_GIF ? "gif" : "jpg";
            ...
            if (file_exists($dest_path)) 
            {
                ...
                if ($size[2] != IMG_GIF && $size[2] != IMG_JPG) // gif 파일이 아니면 올라간 이미지를 삭제한다.
                    @unlink($dest_path); 
                else
...
$mb_dir2 = "$g4[path]/data/member/".md5(substr($mb_id, 0, 2));
if (is_uploaded_file($_FILES[mb_certificate][tmp_name])){
    //@mkdir("$g4[path]/data/member/".md5(substr($mb_id, 0, 2));
	@mkdir($mb_dir2, 0707);
	@chmod($mb_dir2, 0707);
	$imgtype2 = getimagesize($_FILES[mb_certificate][tmp_name]);
	$ext2 = "jpg";
	$certificate_path = "{$mb_dir2}/{$mb_id}_{$_FILES[mb_certificate][name]}";
	move_uploaded_file($_FILES[mb_certificate][tmp_name], $certificate_path);
	chmod($certificate_path, 0606);

	$mb_5 = $_FILES[mb_certificate][name];
}
...
if ($w == "") 
{
    ...
    $sql = " insert into $g4[member_table]
                set mb_id = '$mb_id',
                    ...
                    mb_9 = '$mb_9',
                    mb_10 = '$mb_10' 
                    , mb_is_naver = '$mb_is_naver'
					, mb_naver_id = '$mb_naver_id'
					, mb_is_kakao = '$mb_is_kakao'
					, mb_kakao_id = '$mb_kakao_id'
					, mb_is_facebook = '$mb_is_facebook'

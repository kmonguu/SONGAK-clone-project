...
if (get_cookie('ck_visit_ip') != $_SERVER['REMOTE_ADDR']) {
   ...
   if(is_mobile()){
    ...
    $sql = " insert $g4[visit_table] ( vi_id, vi_ip, vi_date, vi_time, vi_referer, vi_agent, is_mobile ) values ( '$vi_id', '$remote_addr', '$g4[time_ymd]', '$g4[time_his]', '$referer', '$user_agent', '$is_mobile' ) ";
	...
	if($is_mobile == "Y")
	        $sql = " insert $g4[visit_sum_table] ( vs_count, vs_mobile, vs_date) values ( 1, 1, '$g4[time_ymd]' ) ";
	else 
		$sql = " insert $g4[visit_sum_table] ( vs_count, vs_pc, vs_date) values ( 1, 1, '$g4[time_ymd]' ) ";
        ...
        if (!$result) {
		    if($is_mobile == "Y")
			    $sql = " update $g4[visit_sum_table] set vs_count = vs_count + 1, vs_mobile = vs_mobile +1 where vs_date = '$g4[time_ymd]' ";
		    else
			    $sql = " update $g4[visit_sum_table] set vs_count = vs_count + 1, vs_pc = vs_pc+1 where vs_date = '$g4[time_ymd]' ";

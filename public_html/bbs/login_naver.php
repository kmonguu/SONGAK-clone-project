...
if($r->error){
	if($r->error == "invalid_request") {
		echo "<script>alert('[{$r->error}] 취소되었습니다.');window.close();</script>";
	} else {
		echo "<script>alert('{$r->error}');window.close();</script>";
	}
	exit;
} 
...
if($n_member[mb_id]) {
	echo "
		<form name='naver_loginform' method='post' action='/bbs/login_naver_check.php'/>
			<input type='hidden' name='mb_id' value='{$n_member[mb_id]}'/>
			<input type='hidden' name='mb_password' value='{$user->id}'/>
			<input type='hidden' name='access_token' value='{$access_token}'/>
		</form>
		<script>
			opener.name = '{$access_token}';
			document.naver_loginform.target = opener.name;
...
echo "
<form name='naver_loginform' method='post' action='/bbs/register_naver.php'/>

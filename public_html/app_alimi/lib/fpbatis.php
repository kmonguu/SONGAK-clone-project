<?php
	...
	class FPBatis_forAlimi {
		...
		function createConnection() {
			...
			$server[0] = $mysql_host;
			$properties['JDBC.Username'] = $mysql_user;
			$properties['JDBC.Password'] = $mysql_password;
			*/
			if($_SESSION["chat_server"] && $useChatDB == "Y") {
				
				$this->conn = @mysql_connect($_SESSION["chat_server"], "ninetalk", "ninetalk1937admin") or die('Unable to connect to server.');
				//if (!empty($serverSpecs[3]))
				mysql_select_db("ninetalk", $this->conn) or die('Unable to connect to database.');
			...
			foreach ($elm->childNodes as $item) {
				switch($item->nodeName) {
					...
					case 'isNotNull':
						...
						break;
					case 'isEqual':
						if($params[$item->getAttribute('property')] == $item->getAttribute('compareValue')){
							$stmt .= $this->applyDynamicElement($item, $params, $dynamic);
							$dynamic = false;
						}
						break;
		...
		function doInsert($id, $obj, $fromForm=false) {
			...
			if ($elm = $this->findMapElement($ids[0], 'insert', $ids[1])) {
				...
				$stmt = $pieces[0];
				for ($i = 1; $i < sizeof($pieces); $i+=2) {
					if ($fromForm) $obj[$pieces[$i]] = $this->param($pieces[$i]);

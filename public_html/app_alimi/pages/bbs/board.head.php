...
function alert_app($msg='', $url='')
{
	...
	echo "</script>";
	if ($url) {
		echo "<script type='text/javascript'>go_link_page('{$url}')</script>";
	}
	exit;
}
...
if ($wr_id)
{
    ...
    if (!$write[wr_id])
    {
        ...
        if ($cwin)
            ...
            //alert_app($msg);//, "$g4[app_path]/pages.php?p=1_1_1_1&bo_table=$bo_table");
            alert_app($msg, "p=1_1_1_1&bo_table=$bo_table");
        }
    }
    ...
    if ($group[gr_use_access])
    {
        if (!$member[mb_id]) {
            ...
            if ($cwin)
                ...
                alert($msg, $g4[mpath]."/login.php?wr_id=$wr_id{$qstr}&url=".urlencode("../view.php?bo_table=$bo_table&wr_id=$wr_id"));
        }
    ...
    if ($member[mb_level] < $board[bo_read_level])
    {
        if ($member[mb_id])
            ...
            alert("글을 읽을 권한이 없습니다.\\n\\n회원이시라면 로그인 후 이용해 보십시오.", $g4[mpath]."/login.php?wr_id=$wr_id{$qstr}&url=".urlencode("../view.php?bo_table=$bo_table&wr_id=$wr_id"));
    }

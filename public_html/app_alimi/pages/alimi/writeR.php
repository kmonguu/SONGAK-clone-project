...
<div style="height:20px;"></div>
	<ul>
		...
		<? if ($w != "r") { ?>
		<li style='font-weight:bold'>
		예약현황<br/>1:예약접수, 2:예약처리중, 3:예약완료
		</li>
		<li>
		<input class="" size="15" name="wr_10" itemname="예약현황" required="required" placeholder="예약현황" value="<?=$write[wr_10]?>" onFocus="this.placeholder=''">
		</li>
		<?}else{?>
			<li><input type='hidden' value='<?=$write[wr_10]?>'></li>
		<?}?>
		...
		<li>
			<input class='' type=text id=wr_3 name='wr_3' style="width:105px;" maxlength=20  itemname='숙박기간' required value='<?=$write[wr_3]?>' readonly title='오시는 날짜를 입력하세요.'>
			<select id="select_box" class=input_txt1 name='wr_2' class='ed' required itemname='숙박기간' style="font-size:14px;">
				<option value='1박2일' <? if($write[wr_2] == "1박2일") echo "selected"; ?> >1박2일</option>
				<option value='2박3일' <? if($write[wr_2] == "2박3일") echo "selected"; ?>>2박3일</option>
				<option value='3박4일' <? if($write[wr_2] == "3박4일") echo "selected"; ?>>3박4일</option>
				<option value='4박5일' <? if($write[wr_2] == "double2") echo "selected"; ?>>4박5일</option>
			</select>
		</li>
		...
		<li>
			성인:<input class="" size="4" name="wr_5" itemname="성인" required="required" placeholder="성인" value="<?=$write[wr_5]?>" onFocus="this.placeholder=''" style="width:50px"> 
			소아:<input class="" size="4" name="wr_6" itemname="성인" required="required" placeholder="성인" value="<?=$write[wr_6]?>" onFocus="this.placeholder=''" style="width:50px">
		</li>
...
function fwrite_submit(f) {
   ...
   if(f.wr_1.value.split("-").length != 3){
	alert("핸드폰 형식을 올바르게 입력해주십시오 ex. 000-0000-0000");
	f.wr_1.focus();
	return false;
   }

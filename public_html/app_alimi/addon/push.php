...
if(is_file("{$g4["path"]}/app_helper/app.php")) {
	...
	$fcmObj = new HpFcm();
	$fcmObj->send($_REQUEST["push_title"], $_REQUEST["push_message"], array(), "chatting", $room_no, "chatting");
}


function sendPush($push_title, $push_msg, $room_no=""){
...
$ss_push_key = $_SESSION["ss_push_key"];

if($ss_push_key == md5($_REQUEST["chat_id"]."push_key")){ 
	
	sendPush($_REQUEST["push_title"], $_REQUEST["push_message"], $_REQUEST["room_no"]);
}

$_SESSION["ss_push_key"] = "";
